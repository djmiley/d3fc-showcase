 ...
 */
var computeLayout=function(){function capitalizeFirst(str){return str.charAt(0).toUpperCase()+str.slice(1)}function getSpacing(node,type,suffix,locations){for(var i=0;i<locations.length;++i){var location=locations[i],key=type+capitalizeFirst(location)+suffix;if(key in node.style)return node.style[key];if(key=type+suffix,key in node.style)return node.style[key]}return 0}
// When transpiled to Java / C the node type has layout, children and style
// properties. For the JavaScript version this function adds these properties
// if they don't already exist.
function fillNodes(node){return node.layout||(node.layout={width:void 0,height:void 0,top:0,left:0,right:0,bottom:0}),node.style||(node.style={}),node.children||(node.children=[]),node.children.forEach(fillNodes),node}function getPositiveSpacing(node,type,suffix,locations){for(var i=0;i<locations.length;++i){var location=locations[i],key=type+capitalizeFirst(location)+suffix;if(key in node.style&&node.style[key]>=0)return node.style[key];if(key=type+suffix,key in node.style&&node.style[key]>=0)return node.style[key]}return 0}function isUndefined(value){return void 0===value}function isRowDirection(flexDirection){return flexDirection===CSS_FLEX_DIRECTION_ROW||flexDirection===CSS_FLEX_DIRECTION_ROW_REVERSE}function isColumnDirection(flexDirection){return flexDirection===CSS_FLEX_DIRECTION_COLUMN||flexDirection===CSS_FLEX_DIRECTION_COLUMN_REVERSE}function getLeadingLocations(axis){var locations=[leading[axis]];return isRowDirection(axis)&&locations.unshift("start"),locations}function getTrailingLocations(axis){var locations=[trailing[axis]];return isRowDirection(axis)&&locations.unshift("end"),locations}function getMargin(node,locations){return getSpacing(node,"margin","",locations)}function getLeadingMargin(node,axis){return getMargin(node,getLeadingLocations(axis))}function getTrailingMargin(node,axis){return getMargin(node,getTrailingLocations(axis))}function getPadding(node,locations){return getPositiveSpacing(node,"padding","",locations)}function getLeadingPadding(node,axis){return getPadding(node,getLeadingLocations(axis))}function getTrailingPadding(node,axis){return getPadding(node,getTrailingLocations(axis))}function getBorder(node,locations){return getPositiveSpacing(node,"border","Width",locations)}function getLeadingBorder(node,axis){return getBorder(node,getLeadingLocations(axis))}function getTrailingBorder(node,axis){return getBorder(node,getTrailingLocations(axis))}function getLeadingPaddingAndBorder(node,axis){return getLeadingPadding(node,axis)+getLeadingBorder(node,axis)}function getTrailingPaddingAndBorder(node,axis){return getTrailingPadding(node,axis)+getTrailingBorder(node,axis)}function getBorderAxis(node,axis){return getLeadingBorder(node,axis)+getTrailingBorder(node,axis)}function getMarginAxis(node,axis){return getLeadingMargin(node,axis)+getTrailingMargin(node,axis)}function getPaddingAndBorderAxis(node,axis){return getLeadingPaddingAndBorder(node,axis)+getTrailingPaddingAndBorder(node,axis)}function getJustifyContent(node){return"justifyContent"in node.style?node.style.justifyContent:"flex-start"}function getAlignContent(node){return"alignContent"in node.style?node.style.alignContent:"flex-start"}function getAlignItem(node,child){return"alignSelf"in child.style?child.style.alignSelf:"alignItems"in node.style?node.style.alignItems:"stretch"}function resolveAxis(axis,direction){if(direction===CSS_DIRECTION_RTL){if(axis===CSS_FLEX_DIRECTION_ROW)return CSS_FLEX_DIRECTION_ROW_REVERSE;if(axis===CSS_FLEX_DIRECTION_ROW_REVERSE)return CSS_FLEX_DIRECTION_ROW}return axis}function resolveDirection(node,parentDirection){var direction;return direction="direction"in node.style?node.style.direction:CSS_DIRECTION_INHERIT,direction===CSS_DIRECTION_INHERIT&&(direction=void 0===parentDirection?CSS_DIRECTION_LTR:parentDirection),direction}function getFlexDirection(node){return"flexDirection"in node.style?node.style.flexDirection:CSS_FLEX_DIRECTION_COLUMN}function getCrossFlexDirection(flexDirection,direction){return isColumnDirection(flexDirection)?resolveAxis(CSS_FLEX_DIRECTION_ROW,direction):CSS_FLEX_DIRECTION_COLUMN}function getPositionType(node){return"position"in node.style?node.style.position:"relative"}function getFlex(node){return node.style.flex}function isFlex(node){return getPositionType(node)===CSS_POSITION_RELATIVE&&getFlex(node)>0}function isFlexWrap(node){return"wrap"===node.style.flexWrap}function getDimWithMargin(node,axis){return node.layout[dim[axis]]+getMarginAxis(node,axis)}function isDimDefined(node,axis){return!isUndefined(node.style[dim[axis]])&&node.style[dim[axis]]>=0}function isPosDefined(node,pos){return!isUndefined(node.style[pos])}function isMeasureDefined(node){return"measure"in node.style}function getPosition(node,pos){return pos in node.style?node.style[pos]:0}function boundAxis(node,axis,value){var min={row:node.style.minWidth,"row-reverse":node.style.minWidth,column:node.style.minHeight,"column-reverse":node.style.minHeight}[axis],max={row:node.style.maxWidth,"row-reverse":node.style.maxWidth,column:node.style.maxHeight,"column-reverse":node.style.maxHeight}[axis],boundValue=value;return!isUndefined(max)&&max>=0&&boundValue>max&&(boundValue=max),!isUndefined(min)&&min>=0&&min>boundValue&&(boundValue=min),boundValue}function fmaxf(a,b){return a>b?a:b}
// When the user specifically sets a value for width or height
function setDimensionFromStyle(node,axis){
// The parent already computed us a width or height. We just skip it
isUndefined(node.layout[dim[axis]])&&isDimDefined(node,axis)&&(
// The dimensions can never be smaller than the padding and border
node.layout[dim[axis]]=fmaxf(boundAxis(node,axis,node.style[dim[axis]]),getPaddingAndBorderAxis(node,axis)))}function setTrailingPosition(node,child,axis){child.layout[trailing[axis]]=node.layout[dim[axis]]-child.layout[dim[axis]]-child.layout[pos[axis]]}
// If both left and right are defined, then use left. Otherwise return
// +left or -right depending on which is defined.
function getRelativePosition(node,axis){return leading[axis]in node.style?getPosition(node,leading[axis]):-getPosition(node,trailing[axis])}function layoutNode(node,parentMaxWidth,/*css_direction_t*/parentDirection){var/*css_direction_t*/direction=resolveDirection(node,parentDirection),/*css_flex_direction_t*/mainAxis=resolveAxis(getFlexDirection(node),direction),/*css_flex_direction_t*/crossAxis=getCrossFlexDirection(mainAxis,direction),/*css_flex_direction_t*/resolvedRowAxis=resolveAxis(CSS_FLEX_DIRECTION_ROW,direction);if(
// Handle width and height style attributes
setDimensionFromStyle(node,mainAxis),setDimensionFromStyle(node,crossAxis),
// Set the resolved resolution in the node's layout
node.layout.direction=direction,
// The position is set by the parent, but we need to complete it with a
// delta composed of the margin and left/top/right/bottom
node.layout[leading[mainAxis]]+=getLeadingMargin(node,mainAxis)+getRelativePosition(node,mainAxis),node.layout[trailing[mainAxis]]+=getTrailingMargin(node,mainAxis)+getRelativePosition(node,mainAxis),node.layout[leading[crossAxis]]+=getLeadingMargin(node,crossAxis)+getRelativePosition(node,crossAxis),node.layout[trailing[crossAxis]]+=getTrailingMargin(node,crossAxis)+getRelativePosition(node,crossAxis),isMeasureDefined(node)){var/*float*/width=CSS_UNDEFINED;width=isDimDefined(node,resolvedRowAxis)?node.style.width:isUndefined(node.layout[dim[resolvedRowAxis]])?parentMaxWidth-getMarginAxis(node,resolvedRowAxis):node.layout[dim[resolvedRowAxis]],width-=getPaddingAndBorderAxis(node,resolvedRowAxis);
// We only need to give a dimension for the text if we haven't got any
// for it computed yet. It can either be from the style attribute or because
// the element is flexible.
var/*bool*/isRowUndefined=!isDimDefined(node,resolvedRowAxis)&&isUndefined(node.layout[dim[resolvedRowAxis]]),/*bool*/isColumnUndefined=!isDimDefined(node,CSS_FLEX_DIRECTION_COLUMN)&&isUndefined(node.layout[dim[CSS_FLEX_DIRECTION_COLUMN]]);
// Let's not measure the text if we already know both dimensions
if(isRowUndefined||isColumnUndefined){var/*css_dim_t*/measureDim=node.style.measure(/*(c)!node->context,*/
/*(java)!layoutContext.measureOutput,*/
width);isRowUndefined&&(node.layout.width=measureDim.width+getPaddingAndBorderAxis(node,resolvedRowAxis)),isColumnUndefined&&(node.layout.height=measureDim.height+getPaddingAndBorderAxis(node,CSS_FLEX_DIRECTION_COLUMN))}if(0===node.children.length)return}var/*int*/i,/*int*/ii,/*css_node_t**/child,/*css_flex_direction_t*/axis;
// Pre-fill some dimensions straight from the parent
for(i=0;i<node.children.length;++i)
// Pre-fill cross axis dimensions when the child is using stretch before
// we call the recursive layout pass
if(child=node.children[i],getAlignItem(node,child)!==CSS_ALIGN_STRETCH||getPositionType(child)!==CSS_POSITION_RELATIVE||isUndefined(node.layout[dim[crossAxis]])||isDimDefined(child,crossAxis)){if(getPositionType(child)===CSS_POSITION_ABSOLUTE)
// Pre-fill dimensions when using absolute position and both offsets for the axis are defined (either both
// left and right or top and bottom).
for(ii=0;2>ii;ii++)axis=0!==ii?CSS_FLEX_DIRECTION_ROW:CSS_FLEX_DIRECTION_COLUMN,!isUndefined(node.layout[dim[axis]])&&!isDimDefined(child,axis)&&isPosDefined(child,leading[axis])&&isPosDefined(child,trailing[axis])&&(child.layout[dim[axis]]=fmaxf(boundAxis(child,axis,node.layout[dim[axis]]-getPaddingAndBorderAxis(node,axis)-getMarginAxis(child,axis)-getPosition(child,leading[axis])-getPosition(child,trailing[axis])),getPaddingAndBorderAxis(child,axis)))}else child.layout[dim[crossAxis]]=fmaxf(boundAxis(child,crossAxis,node.layout[dim[crossAxis]]-getPaddingAndBorderAxis(node,crossAxis)-getMarginAxis(child,crossAxis)),
// You never want to go smaller than padding
getPaddingAndBorderAxis(child,crossAxis));var/*float*/definedMainDim=CSS_UNDEFINED;isUndefined(node.layout[dim[mainAxis]])||(definedMainDim=node.layout[dim[mainAxis]]-getPaddingAndBorderAxis(node,mainAxis));for(
// We want to execute the next two loops one per line with flex-wrap
var/*int*/startLine=0,/*int*/endLine=0,/*int*/alreadyComputedNextLayout=0,/*float*/linesCrossDim=0,/*float*/linesMainDim=0,/*int*/linesCount=0;endLine<node.children.length;){
// <Loop A> Layout non flexible children and count children by type
// mainContentDim is accumulation of the dimensions and margin of all the
// non flexible children. This will be used in order to either set the
// dimensions of the node if none already exist, or to compute the
// remaining space left for the flexible children.
var/*float*/maxWidth,/*float*/mainContentDim=0,/*int*/flexibleChildrenCount=0,/*float*/totalFlexible=0,/*int*/nonFlexibleChildrenCount=0;for(i=startLine;i<node.children.length;++i){child=node.children[i];var/*float*/nextContentDim=0;
// The element we are about to add would make us go to the next line
if(
// It only makes sense to consider a child flexible if we have a computed
// dimension for the node.
!isUndefined(node.layout[dim[mainAxis]])&&isFlex(child)?(flexibleChildrenCount++,totalFlexible+=getFlex(child),nextContentDim=getPaddingAndBorderAxis(child,mainAxis)+getMarginAxis(child,mainAxis)):(maxWidth=CSS_UNDEFINED,isRowDirection(mainAxis)||(maxWidth=parentMaxWidth-getMarginAxis(node,resolvedRowAxis)-getPaddingAndBorderAxis(node,resolvedRowAxis),isDimDefined(node,resolvedRowAxis)&&(maxWidth=node.layout[dim[resolvedRowAxis]]-getPaddingAndBorderAxis(node,resolvedRowAxis))),0===alreadyComputedNextLayout&&layoutNode(child,maxWidth,direction),getPositionType(child)===CSS_POSITION_RELATIVE&&(nonFlexibleChildrenCount++,nextContentDim=getDimWithMargin(child,mainAxis))),isFlexWrap(node)&&!isUndefined(node.layout[dim[mainAxis]])&&mainContentDim+nextContentDim>definedMainDim&&i!==startLine){nonFlexibleChildrenCount--,alreadyComputedNextLayout=1;break}alreadyComputedNextLayout=0,mainContentDim+=nextContentDim,endLine=i+1}
// <Loop B> Layout flexible children and allocate empty space
// In order to position the elements in the main axis, we have two
// controls. The space between the beginning and the first element
// and the space between each two elements.
var/*float*/leadingMainDim=0,/*float*/betweenMainDim=0,/*float*/remainingMainDim=0;
// If there are flexible children in the mix, they are going to fill the
// remaining space
if(remainingMainDim=isUndefined(node.layout[dim[mainAxis]])?fmaxf(mainContentDim,0)-mainContentDim:definedMainDim-mainContentDim,0!==flexibleChildrenCount){var/*float*/baseMainDim,/*float*/boundMainDim,/*float*/flexibleMainDim=remainingMainDim/totalFlexible;
...
// calculations.
for(i=startLine;endLine>i;++i)child=node.children[i],isFlex(child)&&(baseMainDim=flexibleMainDim*getFlex(child)+getPaddingAndBorderAxis(child,mainAxis),boundMainDim=boundAxis(child,mainAxis,baseMainDim),baseMainDim!==boundMainDim&&(remainingMainDim-=boundMainDim,totalFlexible-=getFlex(child)));
// We iterate over the full array and only apply the action on flexible
// children. This is faster than actually allocating a new array that
// contains only flexible children.
for(flexibleMainDim=remainingMainDim/totalFlexible,0>flexibleMainDim&&(flexibleMainDim=0),i=startLine;endLine>i;++i)child=node.children[i],isFlex(child)&&(child.layout[dim[mainAxis]]=boundAxis(child,mainAxis,flexibleMainDim*getFlex(child)+getPaddingAndBorderAxis(child,mainAxis)),maxWidth=CSS_UNDEFINED,isDimDefined(node,resolvedRowAxis)?maxWidth=node.layout[dim[resolvedRowAxis]]-getPaddingAndBorderAxis(node,resolvedRowAxis):isRowDirection(mainAxis)||(maxWidth=parentMaxWidth-getMarginAxis(node,resolvedRowAxis)-getPaddingAndBorderAxis(node,resolvedRowAxis)),layoutNode(child,maxWidth,direction))}else{var/*css_justify_t*/justifyContent=getJustifyContent(node);justifyContent===CSS_JUSTIFY_CENTER?leadingMainDim=remainingMainDim/2:justifyContent===CSS_JUSTIFY_FLEX_END?leadingMainDim=remainingMainDim:justifyContent===CSS_JUSTIFY_SPACE_BETWEEN?(remainingMainDim=fmaxf(remainingMainDim,0),betweenMainDim=flexibleChildrenCount+nonFlexibleChildrenCount-1!==0?remainingMainDim/(flexibleChildrenCount+nonFlexibleChildrenCount-1):0):justifyContent===CSS_JUSTIFY_SPACE_AROUND&&(betweenMainDim=remainingMainDim/(flexibleChildrenCount+nonFlexibleChildrenCount),leadingMainDim=betweenMainDim/2)}
// <Loop C> Position elements in the main axis and compute dimensions
// At this point, all the children have their dimensions set. We need to
// find their position. In order to do that, we accumulate data in
// variables that are also useful to compute the total dimensions of the
// container!
var/*float*/crossDim=0,/*float*/mainDim=leadingMainDim+getLeadingPaddingAndBorder(node,mainAxis);for(i=startLine;endLine>i;++i)child=node.children[i],child.lineIndex=linesCount,getPositionType(child)===CSS_POSITION_ABSOLUTE&&isPosDefined(child,leading[mainAxis])?child.layout[pos[mainAxis]]=getPosition(child,leading[mainAxis])+getLeadingBorder(node,mainAxis)+getLeadingMargin(child,mainAxis):(child.layout[pos[mainAxis]]+=mainDim,isUndefined(node.layout[dim[mainAxis]])||setTrailingPosition(node,child,mainAxis)),getPositionType(child)===CSS_POSITION_RELATIVE&&(mainDim+=betweenMainDim+getDimWithMargin(child,mainAxis),crossDim=fmaxf(crossDim,boundAxis(child,crossAxis,getDimWithMargin(child,crossAxis))));var/*float*/containerCrossAxis=node.layout[dim[crossAxis]];
// <Loop D> Position elements in the cross axis
for(isUndefined(node.layout[dim[crossAxis]])&&(containerCrossAxis=fmaxf(
// For the cross dim, we add both sides at the end because the value
// is aggregate via a max function. Intermediate negative values
// can mess this computation otherwise
boundAxis(node,crossAxis,crossDim+getPaddingAndBorderAxis(node,crossAxis)),getPaddingAndBorderAxis(node,crossAxis))),i=startLine;endLine>i;++i)if(child=node.children[i],getPositionType(child)===CSS_POSITION_ABSOLUTE&&isPosDefined(child,leading[crossAxis]))
// In case the child is absolutely positionned and has a
// top/left/bottom/right being set, we override all the previously
// computed positions to set it correctly.
child.layout[pos[crossAxis]]=getPosition(child,leading[crossAxis])+getLeadingBorder(node,crossAxis)+getLeadingMargin(child,crossAxis);else{var/*float*/leadingCrossDim=getLeadingPaddingAndBorder(node,crossAxis);
// For a relative children, we're either using alignItems (parent) or
// alignSelf (child) in order to determine the position in the cross axis
if(getPositionType(child)===CSS_POSITION_RELATIVE){var/*css_align_t*/alignItem=getAlignItem(node,child);if(alignItem===CSS_ALIGN_STRETCH)
// You can only stretch if the dimension has not already been set
// previously.
isDimDefined(child,crossAxis)||(child.layout[dim[crossAxis]]=fmaxf(boundAxis(child,crossAxis,containerCrossAxis-getPaddingAndBorderAxis(node,crossAxis)-getMarginAxis(child,crossAxis)),
// You never want to go smaller than padding
getPaddingAndBorderAxis(child,crossAxis)));else if(alignItem!==CSS_ALIGN_FLEX_START){
// The remaining space between the parent dimensions+padding and child
// dimensions+margin.
var/*float*/remainingCrossDim=containerCrossAxis-getPaddingAndBorderAxis(node,crossAxis)-getDimWithMargin(child,crossAxis);leadingCrossDim+=alignItem===CSS_ALIGN_CENTER?remainingCrossDim/2:remainingCrossDim}}
// And we apply the position
child.layout[pos[crossAxis]]+=linesCrossDim+leadingCrossDim,
// Define the trailing position accordingly.
isUndefined(node.layout[dim[crossAxis]])||setTrailingPosition(node,child,crossAxis)}linesCrossDim+=crossDim,linesMainDim=fmaxf(linesMainDim,mainDim),linesCount+=1,startLine=endLine}
// <Loop E>
//
...
// section 9.4
//
if(linesCount>1&&!isUndefined(node.layout[dim[crossAxis]])){var/*float*/nodeCrossAxisInnerSize=node.layout[dim[crossAxis]]-getPaddingAndBorderAxis(node,crossAxis),/*float*/remainingAlignContentDim=nodeCrossAxisInnerSize-linesCrossDim,/*float*/crossDimLead=0,/*float*/currentLead=getLeadingPaddingAndBorder(node,crossAxis),/*css_align_t*/alignContent=getAlignContent(node);alignContent===CSS_ALIGN_FLEX_END?currentLead+=remainingAlignContentDim:alignContent===CSS_ALIGN_CENTER?currentLead+=remainingAlignContentDim/2:alignContent===CSS_ALIGN_STRETCH&&nodeCrossAxisInnerSize>linesCrossDim&&(crossDimLead=remainingAlignContentDim/linesCount);var/*int*/endIndex=0;for(i=0;linesCount>i;++i){var/*int*/startIndex=endIndex,/*float*/lineHeight=0;for(ii=startIndex;ii<node.children.length;++ii)if(child=node.children[ii],getPositionType(child)===CSS_POSITION_RELATIVE){if(child.lineIndex!==i)break;isUndefined(child.layout[dim[crossAxis]])||(lineHeight=fmaxf(lineHeight,child.layout[dim[crossAxis]]+getMarginAxis(child,crossAxis)))}for(endIndex=ii,lineHeight+=crossDimLead,ii=startIndex;endIndex>ii;++ii)if(child=node.children[ii],getPositionType(child)===CSS_POSITION_RELATIVE){var/*css_align_t*/alignContentAlignItem=getAlignItem(node,child);if(alignContentAlignItem===CSS_ALIGN_FLEX_START)child.layout[pos[crossAxis]]=currentLead+getLeadingMargin(child,crossAxis);else if(alignContentAlignItem===CSS_ALIGN_FLEX_END)child.layout[pos[crossAxis]]=currentLead+lineHeight-getTrailingMargin(child,crossAxis)-child.layout[dim[crossAxis]];else if(alignContentAlignItem===CSS_ALIGN_CENTER){var/*float*/childHeight=child.layout[dim[crossAxis]];child.layout[pos[crossAxis]]=currentLead+(lineHeight-childHeight)/2}else alignContentAlignItem===CSS_ALIGN_STRETCH&&(child.layout[pos[crossAxis]]=currentLead+getLeadingMargin(child,crossAxis))}currentLead+=lineHeight}}var/*bool*/needsMainTrailingPos=!1,/*bool*/needsCrossTrailingPos=!1;
// <Loop F> Set trailing position if necessary
if(
// If the user didn't specify a width or height, and it has not been set
// by the container, then we set it via the children.
isUndefined(node.layout[dim[mainAxis]])&&(node.layout[dim[mainAxis]]=fmaxf(
// We're missing the last padding at this point to get the final
// dimension
boundAxis(node,mainAxis,linesMainDim+getTrailingPaddingAndBorder(node,mainAxis)),
// We can never assign a width smaller than the padding and borders
getPaddingAndBorderAxis(node,mainAxis)),needsMainTrailingPos=!0),isUndefined(node.layout[dim[crossAxis]])&&(node.layout[dim[crossAxis]]=fmaxf(
// For the cross dim, we add both sides at the end because the value
// is aggregate via a max function. Intermediate negative values
// can mess this computation otherwise
boundAxis(node,crossAxis,linesCrossDim+getPaddingAndBorderAxis(node,crossAxis)),getPaddingAndBorderAxis(node,crossAxis)),needsCrossTrailingPos=!0),needsMainTrailingPos||needsCrossTrailingPos)for(i=0;i<node.children.length;++i)child=node.children[i],needsMainTrailingPos&&setTrailingPosition(node,child,mainAxis),needsCrossTrailingPos&&setTrailingPosition(node,child,crossAxis);
// <Loop G> Calculate dimensions for absolutely positioned elements
for(i=0;i<node.children.length;++i)if(child=node.children[i],getPositionType(child)===CSS_POSITION_ABSOLUTE){
// Pre-fill dimensions when using absolute position and both offsets for the axis are defined (either both
// left and right or top and bottom).
for(ii=0;2>ii;ii++)axis=0!==ii?CSS_FLEX_DIRECTION_ROW:CSS_FLEX_DIRECTION_COLUMN,!isUndefined(node.layout[dim[axis]])&&!isDimDefined(child,axis)&&isPosDefined(child,leading[axis])&&isPosDefined(child,trailing[axis])&&(child.layout[dim[axis]]=fmaxf(boundAxis(child,axis,node.layout[dim[axis]]-getBorderAxis(node,axis)-getMarginAxis(child,axis)-getPosition(child,leading[axis])-getPosition(child,trailing[axis])),getPaddingAndBorderAxis(child,axis)));for(ii=0;2>ii;ii++)axis=0!==ii?CSS_FLEX_DIRECTION_ROW:CSS_FLEX_DIRECTION_COLUMN,isPosDefined(child,trailing[axis])&&!isPosDefined(child,leading[axis])&&(child.layout[leading[axis]]=node.layout[dim[axis]]-child.layout[dim[axis]]-getPosition(child,trailing[axis]))}}var CSS_UNDEFINED,CSS_DIRECTION_INHERIT="inherit",CSS_DIRECTION_LTR="ltr",CSS_DIRECTION_RTL="rtl",CSS_FLEX_DIRECTION_ROW="row",CSS_FLEX_DIRECTION_ROW_REVERSE="row-reverse",CSS_FLEX_DIRECTION_COLUMN="column",CSS_FLEX_DIRECTION_COLUMN_REVERSE="column-reverse",CSS_JUSTIFY_CENTER="center",CSS_JUSTIFY_FLEX_END="flex-end",CSS_JUSTIFY_SPACE_BETWEEN="space-between",CSS_JUSTIFY_SPACE_AROUND="space-around",CSS_ALIGN_FLEX_START="flex-start",CSS_ALIGN_CENTER="center",CSS_ALIGN_FLEX_END="flex-end",CSS_ALIGN_STRETCH="stretch",CSS_POSITION_RELATIVE="relative",CSS_POSITION_ABSOLUTE="absolute",leading={row:"left","row-reverse":"right",column:"top","column-reverse":"bottom"},trailing={row:"right","row-reverse":"left",column:"bottom","column-reverse":"top"},pos={row:"left","row-reverse":"right",column:"top","column-reverse":"bottom"},dim={row:"width","row-reverse":"width",column:"height","column-reverse":"height"};return{computeLayout:layoutNode,fillNodes:fillNodes}}();
// This module export is only used for the purposes of unit testing this file. When
// the library is packaged this file is included within css-layout.js which forms 
// the public API.
return"object"==typeof exports&&(module.exports=computeLayout),function(node){computeLayout.fillNodes(node),computeLayout.computeLayout(node)}}),/**
 ...
 */
function(SVGElement){if(SVGElement&&!("innerHTML"in SVGElement.prototype)){var serializeXML=function(node,output){var nodeType=node.nodeType;if(3==nodeType)// TEXT nodes.
// Replace special XML characters with their entities.
output.push(node.textContent.replace(/&/,"&amp;").replace(/</,"&lt;").replace(">","&gt;"));else if(1==nodeType){if(// ELEMENT nodes.
// Serialize Element nodes.
output.push("<",node.tagName),node.hasAttributes())for(var attrMap=node.attributes,i=0,len=attrMap.length;len>i;++i){var attrNode=attrMap.item(i);output.push(" ",attrNode.name,"='",attrNode.value,"'")}if(node.hasChildNodes()){output.push(">");for(var childNodes=node.childNodes,i=0,len=childNodes.length;len>i;++i)serializeXML(childNodes.item(i),output);output.push("</",node.tagName,">")}else output.push("/>")}else{if(8!=nodeType)
...
Object.defineProperty(SVGElement.prototype,"innerHTML",{get:function(){for(var output=[],childNode=this.firstChild;childNode;)serializeXML(childNode,output),childNode=childNode.nextSibling;return output.join("")},set:function(markupText){
// Wipe out the current contents of the element.
for(;this.firstChild;)this.removeChild(this.firstChild);try{
// Parse the markup into valid nodes.
var dXML=new DOMParser;dXML.async=!1,
// Wrap the markup into a SVG node to ensure parsing works.
sXML="<svg xmlns='http://www.w3.org/2000/svg'>"+markupText+"</svg>";for(var svgDocElement=dXML.parseFromString(sXML,"text/xml").documentElement,childNode=svgDocElement.firstChild;childNode;)this.appendChild(this.ownerDocument.importNode(childNode,!0)),childNode=childNode.nextSibling}catch(e){throw new Error("Error parsing XML string")}}})}}((1,eval)("this").SVGElement),function(global,factory){"object"==typeof exports&&"undefined"!=typeof module?module.exports=factory(require("css-layout"),require("d3"),require("svg-innerhtml")):"function"==typeof define&&define.amd?define(["css-layout","d3","svg-innerhtml"],factory):global.fc=factory(global.computeLayout,global.d3,global.svg_innerhtml)}(this,function(computeLayout,d3,svg_innerhtml){"use strict";
// returns the width and height of the given element minus the padding.
function innerDimensions(element){var style=element.ownerDocument.defaultView.getComputedStyle(element);return{width:parseFloat(style.width)-parseFloat(style.paddingLeft)-parseFloat(style.paddingRight),height:parseFloat(style.height)-parseFloat(style.paddingTop)-parseFloat(style.paddingBottom)}}function ownerSVGElement(node){for(;node.ownerSVGElement;)node=node.ownerSVGElement;return node}
// parses the style attribute, converting it into a JavaScript object
function parseStyle(style){if(!style)return{};var properties=style.split(";"),json={};return properties.forEach(function(property){var components=property.split(":");if(2===components.length){var name=components[0].trim(),value=components[1].trim();json[name]=isNaN(value)?value:Number(value)}}),json}
// creates the structure required by the layout engine
function createNodes(el){function getChildNodes(){for(var children=[],i=0;i<el.childNodes.length;i++){var child=el.childNodes[i];1===child.nodeType&&child.getAttribute("layout-style")&&children.push(createNodes(child))}return children}return{style:parseStyle(el.getAttribute("layout-style")),children:getChildNodes(el),element:el}}
...
subtree&&(node.element.setAttribute("layout-width",node.layout.width),node.element.setAttribute("layout-height",node.layout.height)),node.element.setAttribute("layout-x",node.layout.left),node.element.setAttribute("layout-y",node.layout.top),node.element.nodeName.match(/(?:svg|rect)/i)?(node.element.setAttribute("width",node.layout.width),node.element.setAttribute("height",node.layout.height),node.element.setAttribute("x",node.layout.left),node.element.setAttribute("y",node.layout.top)):node.element.setAttribute("transform","translate("+node.layout.left+", "+node.layout.top+")"),node.children.forEach(function(childNode){applyLayout(childNode,!0)})}function computeDimensions(node){return node.hasAttribute("layout-width")&&node.hasAttribute("layout-height")?{width:Number(node.getAttribute("layout-width")),height:Number(node.getAttribute("layout-height"))}:innerDimensions(node)}function computePosition(node){return node.hasAttribute("layout-x")&&node.hasAttribute("layout-y")?{x:Number(node.getAttribute("layout-x")),y:Number(node.getAttribute("layout-y"))}:{x:0,y:0}}function layout(node){if("suspended"!==ownerSVGElement(node).__layout__){var dimensions=computeDimensions(node),position=computePosition(node),layoutNodes=createNodes(node);
// set the dimensions / position of the root
layoutNodes.style.width=dimensions.width,layoutNodes.style.height=dimensions.height,layoutNodes.style.left=position.x,layoutNodes.style.top=position.y,
// use the Facebook CSS goodness
computeLayout(layoutNodes),
// apply the resultant layout
applyLayout(layoutNodes)}}function layoutSuspended(x){return arguments.length?this.each(function(){ownerSVGElement(this).__layout__=x?"suspended":""}):Boolean(ownerSVGElement(this.node()).__layout__)}function layoutSelection(name,value){var argsLength=arguments.length;
// For layout(string), return the lyout value for the first node
if(1===argsLength&&"string"==typeof name){var node=this.node();return Number(node.getAttribute("layout-"+name))}
// for all other invocations, iterate over each item in the selection
return this.each(function(){if(2===argsLength)"string"!=typeof name?(
// layout(number, number) - sets the width and height and performs layout
this.setAttribute("layout-width",name),this.setAttribute("layout-height",value),layout(this)):
// layout(name, value) - sets a layout- attribute
this.setAttribute("layout-style",name+":"+value);else if(1===argsLength){if("string"!=typeof name){
// layout(object) - sets the layout-style property to the given object
var styleObject=name,layoutCss=Object.keys(styleObject).map(function(property){return property+":"+styleObject[property]}).join(";");this.setAttribute("layout-style",layoutCss)}}else 0===argsLength&&
// layout() - executes layout
layout(this)})}/* global requestAnimationFrame:false */
// Debounce render to only occur once per frame
function render(renderInternal){var rafId=null;return function(){null==rafId&&(rafId=requestAnimationFrame(function(){rafId=null,renderInternal()}))}}function noSnap(xScale,yScale){return function(xPixel,yPixel){
...
var result={xInDomainUnits:!1,x:xPixel,yInDomainUnits:!1,y:yPixel};return xScale.invert&&(result.xInDomainUnits=!0,result.x=xScale.invert(xPixel)),yScale.invert&&(result.yInDomainUnits=!0,result.y=yScale.invert(yPixel)),result}}function pointSnap(xScale,yScale,xValue,yValue,data,objectiveFunction){
// a default function that computes the distance between two points
return objectiveFunction=objectiveFunction||function(x,y,cx,cy){var dx=x-cx,dy=y-cy;return dx*dx+dy*dy},function(xPixel,yPixel){var nearest=data.map(function(d){var diff=objectiveFunction(xPixel,yPixel,xScale(xValue(d)),yScale(yValue(d)));return[diff,d]}).reduce(function(accumulator,value){return accumulator[0]>value[0]?value:accumulator},[Number.MAX_VALUE,null])[1];return{datum:nearest,x:nearest?xValue(nearest):xPixel,xInDomainUnits:Boolean(nearest),y:nearest?yValue(nearest):yPixel,yInDomainUnits:Boolean(nearest)}}}function seriesPointSnap(series,data,objectiveFunction){return function(xPixel,yPixel){var xScale=series.xScale(),yScale=series.yScale(),xValue=series.xValue(),yValue=(series.yValue||series.yCloseValue).call(series);return pointSnap(xScale,yScale,xValue,yValue,data,objectiveFunction)(xPixel,yPixel)}}function seriesPointSnapXOnly(series,data){function objectiveFunction(x,y,cx,cy){var dx=x-cx;return Math.abs(dx)}return seriesPointSnap(series,data,objectiveFunction)}function seriesPointSnapYOnly(series,data){function objectiveFunction(x,y,cx,cy){var dy=y-cy;return Math.abs(dy)}return seriesPointSnap(series,data,objectiveFunction)}function isOrdinal(scale){return scale.rangeExtent}
...
if(!isOrdinal(scale))return scale.range();
...
var scaleRange=scale.range(),extent=scale.rangeExtent();if(scaleRange.length<=1)
// we cannot detect the inverted case if the range (and domain) has
// a single item in it.
return extent;var inverted=scaleRange[0]>scaleRange[1];return inverted?[extent[1],extent[0]]:extent}
...
function setRange(scale,scaleRange){isOrdinal(scale)?scale.rangePoints(scaleRange,1):scale.range(scaleRange)}/**
     ...
     * object names.
     */
function rebind(target,source,mappings){return"object"!=typeof mappings?d3.rebind.apply(d3,arguments):(Object.keys(mappings).forEach(function(targetName){var method=source[mappings[targetName]];if("function"!=typeof method)throw new Error("The method "+mappings[targetName]+" does not exist on the source object");target[targetName]=function(){var value=method.apply(source,arguments);return value===source?target:value}}),target)}function capitalizeFirstLetter(str){return str[0].toUpperCase()+str.slice(1)}/**
     ...
     * be rebound.
     */
function rebindAll(target,source,prefix,exclusions){function exclude(testedProperty){return exclusions.some(function(exclusion){return testedProperty.match(exclusion)})}function reboundPropertyName(inputProperty){return""!==prefix?prefix+capitalizeFirstLetter(inputProperty):inputProperty}prefix="undefined"!=typeof prefix?prefix:"",4===arguments.length&&Array.isArray(exclusions)||(exclusions=Array.prototype.slice.call(arguments,3)),exclusions=exclusions.map(function(exclusion){if("string"==typeof exclusion){if(!source.hasOwnProperty(exclusion))throw new Error("The method "+exclusion+" does not exist on the source object");exclusion=new RegExp("^"+exclusion+"$")}return exclusion});var bindings={};for(var property in source)source.hasOwnProperty(property)&&!exclude(property)&&(bindings[reboundPropertyName(property)]=property);rebind(target,source,bindings)}
...
// by the given factor
function fractionalBarWidth(fraction){return function(pixelValues){
// return some default value if there are not enough datapoints to compute the width
if(pixelValues.length<=1)return 10;pixelValues.sort();
// compute the distance between neighbouring items
var neighbourDistances=d3.pairs(pixelValues).map(function(tuple){return Math.abs(tuple[0]-tuple[1])}),minDistance=d3.min(neighbourDistances);return fraction*minDistance}}function context(){return this}function identity(d){return d}function _index(d,i){return i}function noop(d){}/**
     ...
     * @memberof fc.util
     */
function extent(){var fields=[],extraPoint=null,padding=0,symmetricalAbout=null,extents=function(data){
...
fields=fields.map(function(field){return"string"!=typeof field?field:function(d){return d[field]}});var dataMin=d3.min(data,function(d0){return d3.min(d0,function(d1){return d3.min(fields.map(function(f){return f(d1)}))})}),dataMax=d3.max(data,function(d0){return d3.max(d0,function(d1){return d3.max(fields.map(function(f){return f(d1)}))})}),min=dataMin,max=dataMax;if(null!=symmetricalAbout){var distanceFromMax=Math.abs(max-symmetricalAbout),distanceFromMin=Math.abs(min-symmetricalAbout),halfRange=Math.max(distanceFromMax,distanceFromMin);min=symmetricalAbout-halfRange,max=symmetricalAbout+halfRange}var delta;
// Scale the range for the given padding
if("number"==typeof min&&"number"==typeof max)delta=padding*(max-min)/2,min-=delta,max+=delta;else if("[object Date]"===Object.prototype.toString.call(min)){var oldMin=min.getTime(),oldMax=max.getTime();delta=padding*(oldMax-oldMin)/2,min=new Date(oldMin-delta),max=new Date(oldMax+delta)}
// Return the smallest and largest
// Include the specified point in the range
return null!==extraPoint&&(min>extraPoint?min=extraPoint:extraPoint>max&&(max=extraPoint)),[min,max]};/*
        * @param {array} fields the names of object properties that represent field values, or accessor functions.
        */
return extents.fields=function(x){
// the fields parameter must be an array of field names,
// but we can pass non-array types in
return arguments.length?(Array.isArray(x)||(x=[x]),fields=x,extents):fields},extents.include=function(x){return arguments.length?(extraPoint=x,extents):extraPoint},extents.pad=function(x){return arguments.length?(padding=x,extents):padding},extents.symmetricalAbout=function(x){return arguments.length?(symmetricalAbout=x,extents):symmetricalAbout},extents}
...
function expandMargin(margin){var expandedMargin=margin;return"number"==typeof expandedMargin&&(expandedMargin={top:margin,bottom:margin,left:margin,right:margin}),["top","bottom","left","right"].forEach(function(direction){expandedMargin[direction]||(expandedMargin[direction]=0)}),expandedMargin}
// Wrapper around d3's selectAll/data data-join, which allows decoration of the result.
// This is achieved by appending the element to the enter selection before exposing it.
// A default transition of fade in/out is also implicitly added but can be modified.
function dataJoin(){var selector="g",children=!1,element="g",attr={},key=_index,dataJoin=function(container,data){var joinedData=data||identity;
// Can't use instanceof d3.selection (see #458)
container.selectAll&&container.node||(container=d3.select(container));
// update
var selection=container.selectAll(selector);children&&(
// in order to support nested selections, they can be filtered
// to only return immediate children of the container
selection=selection.filter(function(){return this.parentNode===container.node()}));var updateSelection=selection.data(joinedData,key),enterSelection=updateSelection.enter().insert(element).attr(attr).style("opacity",effectivelyZero),exitSelection=d3.transition(updateSelection.exit()).style("opacity",effectivelyZero).remove();
// when container is a transition, all properties of the transition (which can be interpolated)
// will transition
return updateSelection=d3.transition(updateSelection).style("opacity",1),updateSelection.enter=d3.functor(enterSelection),updateSelection.exit=d3.functor(exitSelection),updateSelection};return dataJoin.selector=function(x){return arguments.length?(selector=x,dataJoin):selector},dataJoin.children=function(x){return arguments.length?(children=x,dataJoin):children},dataJoin.element=function(x){return arguments.length?(element=x,dataJoin):element},dataJoin.attr=function(x){if(!arguments.length)return attr;if(1===arguments.length)attr=arguments[0];else if(2===arguments.length){var dataKey=arguments[0],value=arguments[1];attr[dataKey]=value}return dataJoin},dataJoin.key=function(x){return arguments.length?(key=x,dataJoin):key},dataJoin}function measure(){function updatePositions(){var container=d3.select(this),datum=container.datum()[0];if("DONE"!==datum.state){var mouse=d3.mouse(this),snapped=snap.apply(this,mouse);if("SELECT_SOURCE"===datum.state)datum.source=datum.target=snapped;else{if("SELECT_TARGET"!==datum.state)throw new Error("Unknown state "+datum.state);datum.target=snapped}}}function mouseenter(){var container=d3.select(this).on("click.measure",mouseclick).on("mousemove.measure",mousemove).on("mouseleave.measure",mouseleave),data=container.datum();null==data[0]&&data.push({state:"SELECT_SOURCE"}),updatePositions.call(this),container.call(measure)}function mousemove(){var container=d3.select(this);updatePositions.call(this),container.call(measure)}function mouseleave(){var container=d3.select(this),data=container.datum();null!=data[0]&&"SELECT_SOURCE"===data[0].state&&data.pop(),container.on("click.measure",null).on("mousemove.measure",null).on("mouseleave.measure",null)}function mouseclick(){var container=d3.select(this),datum=container.datum()[0];switch(datum.state){case"SELECT_SOURCE":updatePositions.call(this),event.measuresource.apply(this,arguments),datum.state="SELECT_TARGET";break;case"SELECT_TARGET":updatePositions.call(this),event.measuretarget.apply(this,arguments),datum.state="DONE";break;case"DONE":event.measureclear.apply(this,arguments),datum.state="SELECT_SOURCE",updatePositions.call(this);break;default:throw new Error("Unknown state "+datum.state)}container.call(measure)}var event=d3.dispatch("measuresource","measuretarget","measureclear"),xScale=d3.time.scale(),yScale=d3.scale.linear(),snap=function(_x,_y){return noSnap(xScale,yScale)(_x,_y)},decorate=noop,xLabel=d3.functor(""),yLabel=d3.functor(""),padding=d3.functor(2),x=function(d){return d.xInDomainUnits?xScale(d.x):d.x},y=function(d){return d.yInDomainUnits?yScale(d.y):d.y},dataJoin$$=dataJoin().selector("g.measure").element("g").attr("class","measure"),measure=function(selection){selection.each(function(data,index){var container=d3.select(this).style("pointer-events","all").on("mouseenter.measure",mouseenter),overlay=container.selectAll("rect").data([data]);overlay.enter().append("rect").style("visibility","hidden"),container.select("rect").attr("x",xScale.range()[0]).attr("y",yScale.range()[1]).attr("width",xScale.range()[1]).attr("height",yScale.range()[0]);var g=dataJoin$$(container,data),enter=g.enter();enter.append("line").attr("class","tangent"),enter.append("line").attr("class","horizontal"),enter.append("line").attr("class","vertical"),enter.append("text").attr("class","horizontal"),enter.append("text").attr("class","vertical"),g.select("line.tangent").attr("x1",function(d){return x(d.source)}).attr("y1",function(d){return y(d.source)}).attr("x2",function(d){return x(d.target)}).attr("y2",function(d){return y(d.target)}),g.select("line.horizontal").attr("x1",function(d){return x(d.source)}).attr("y1",function(d){return y(d.source)}).attr("x2",function(d){return x(d.target)}).attr("y2",function(d){return y(d.source)}).style("visibility",function(d){return"DONE"!==d.state?"hidden":"visible"}),g.select("line.vertical").attr("x1",function(d){return x(d.target)}).attr("y1",function(d){return y(d.target)}).attr("x2",function(d){return x(d.target)}).attr("y2",function(d){return y(d.source)}).style("visibility",function(d){return"DONE"!==d.state?"hidden":"visible"});var paddingValue=padding.apply(this,arguments);g.select("text.horizontal").attr("x",function(d){return x(d.source)+(x(d.target)-x(d.source))/2}).attr("y",function(d){return y(d.source)-paddingValue}).style("visibility",function(d){return"DONE"!==d.state?"hidden":"visible"}).text(xLabel),g.select("text.vertical").attr("x",function(d){return x(d.target)+paddingValue}).attr("y",function(d){return y(d.source)+(y(d.target)-y(d.source))/2}).style("visibility",function(d){return"DONE"!==d.state?"hidden":"visible"}).text(yLabel),decorate(g,data,index)})};return measure.xScale=function(_x){return arguments.length?(xScale=_x,measure):xScale},measure.yScale=function(_x){return arguments.length?(yScale=_x,measure):yScale},measure.snap=function(_x){return arguments.length?(snap=_x,measure):snap},measure.decorate=function(_x){return arguments.length?(decorate=_x,measure):decorate},measure.xLabel=function(_x){return arguments.length?(xLabel=d3.functor(_x),measure):xLabel},measure.yLabel=function(_x){return arguments.length?(yLabel=d3.functor(_x),measure):yLabel},measure.padding=function(_x){return arguments.length?(padding=d3.functor(_x),measure):padding},d3.rebind(measure,event,"on"),measure}function fibonacciFan(){function updatePositions(){var container=d3.select(this),datum=container.datum()[0];if("DONE"!==datum.state){var mouse=d3.mouse(this),snapped=snap.apply(this,mouse);if("SELECT_SOURCE"===datum.state)datum.source=datum.target=snapped;else{if("SELECT_TARGET"!==datum.state)throw new Error("Unknown state "+datum.state);datum.target=snapped}}}function mouseenter(){var container=d3.select(this).on("click.fan",mouseclick).on("mousemove.fan",mousemove).on("mouseleave.fan",mouseleave),data=container.datum();null==data[0]&&data.push({state:"SELECT_SOURCE"}),updatePositions.call(this),container.call(fan)}function mousemove(){var container=d3.select(this);updatePositions.call(this),container.call(fan)}function mouseleave(){var container=d3.select(this),data=container.datum();null!=data[0]&&"SELECT_SOURCE"===data[0].state&&data.pop(),container.on("click.fan",null).on("mousemove.fan",null).on("mouseleave.fan",null)}function mouseclick(){var container=d3.select(this),datum=container.datum()[0];switch(datum.state){case"SELECT_SOURCE":updatePositions.call(this),event.fansource.apply(this,arguments),datum.state="SELECT_TARGET";break;case"SELECT_TARGET":updatePositions.call(this),event.fantarget.apply(this,arguments),datum.state="DONE";break;case"DONE":event.fanclear.apply(this,arguments),datum.state="SELECT_SOURCE",updatePositions.call(this);break;default:throw new Error("Unknown state "+datum.state)}container.call(fan)}var event=d3.dispatch("fansource","fantarget","fanclear"),xScale=d3.time.scale(),yScale=d3.scale.linear(),snap=function(_x,_y){return noSnap(xScale,yScale)(_x,_y)},decorate=noop,x=function(d){return d.xInDomainUnits?xScale(d.x):d.x},y=function(d){return d.yInDomainUnits?yScale(d.y):d.y},dataJoin$$=dataJoin().selector("g.fan").element("g").attr("class","fan"),fan=function(selection){selection.each(function(data,index){var container=d3.select(this).style("pointer-events","all").on("mouseenter.fan",mouseenter),overlay=container.selectAll("rect").data([data]);overlay.enter().append("rect").style("visibility","hidden"),container.select("rect").attr("x",xScale.range()[0]).attr("y",yScale.range()[1]).attr("width",xScale.range()[1]).attr("height",yScale.range()[0]);var g=dataJoin$$(container,data);g.each(function(d){if(d.x=xScale.range()[1],d.ay=d.by=d.cy=y(d.target),x(d.source)!==x(d.target)){if("DONE"===d.state&&x(d.source)>x(d.target)){var temp=d.source;d.source=d.target,d.target=temp}var gradient=(y(d.target)-y(d.source))/(x(d.target)-x(d.source)),deltaX=d.x-x(d.source),deltaY=gradient*deltaX;d.ay=.618*deltaY+y(d.source),d.by=.5*deltaY+y(d.source),d.cy=.382*deltaY+y(d.source)}});var enter=g.enter();enter.append("line").attr("class","trend"),enter.append("line").attr("class","a"),enter.append("line").attr("class","b"),enter.append("line").attr("class","c"),enter.append("polygon").attr("class","area"),g.select("line.trend").attr("x1",function(d){return x(d.source)}).attr("y1",function(d){return y(d.source)}).attr("x2",function(d){return x(d.target)}).attr("y2",function(d){return y(d.target)}),g.select("line.a").attr("x1",function(d){return x(d.source)}).attr("y1",function(d){return y(d.source)}).attr("x2",function(d){return d.x}).attr("y2",function(d){return d.ay}).style("visibility",function(d){return"DONE"!==d.state?"hidden":"visible"}),g.select("line.b").attr("x1",function(d){return x(d.source)}).attr("y1",function(d){return y(d.source)}).attr("x2",function(d){return d.x}).attr("y2",function(d){return d.by}).style("visibility",function(d){return"DONE"!==d.state?"hidden":"visible"}),g.select("line.c").attr("x1",function(d){return x(d.source)}).attr("y1",function(d){return y(d.source)}).attr("x2",function(d){return d.x}).attr("y2",function(d){return d.cy}).style("visibility",function(d){return"DONE"!==d.state?"hidden":"visible"}),g.select("polygon.area").attr("points",function(d){return x(d.source)+","+y(d.source)+" "+d.x+","+d.ay+" "+d.x+","+d.cy}).style("visibility",function(d){return"DONE"!==d.state?"hidden":"visible"}),decorate(g,data,index)})};return fan.xScale=function(_x){return arguments.length?(xScale=_x,fan):xScale},fan.yScale=function(_x){return arguments.length?(yScale=_x,fan):yScale},fan.snap=function(_x){return arguments.length?(snap=_x,fan):snap},fan.decorate=function(_x){return arguments.length?(decorate=_x,fan):decorate},d3.rebind(fan,event,"on"),fan}
...
function _multi(){var xScale=d3.time.scale(),yScale=d3.scale.linear(),series=[],mapping=context,key=_index,decorate=noop,dataJoin$$=dataJoin().selector("g.multi").children(!0).attr("class","multi").element("g").key(function(d,i){
...
var dataSeries=this.__series__||d;return key.call(this,dataSeries,i)}),multi=function(selection){selection.each(function(data){var g=dataJoin$$(this,series);g.each(function(dataSeries,i){
...
this.__series__=dataSeries,(dataSeries.xScale||dataSeries.x).call(dataSeries,xScale),(dataSeries.yScale||dataSeries.y).call(dataSeries,yScale),d3.select(this).datum(mapping.call(data,dataSeries,i)).call(dataSeries)}),
// order is not available on a transition selection
d3.selection.prototype.order.call(g),decorate(g)})};return multi.xScale=function(x){return arguments.length?(xScale=x,multi):xScale},multi.yScale=function(x){return arguments.length?(yScale=x,multi):yScale},multi.series=function(x){return arguments.length?(series=x,multi):series},multi.mapping=function(x){return arguments.length?(mapping=x,multi):mapping},multi.key=function(x){return arguments.length?(key=x,multi):key},multi.decorate=function(x){return arguments.length?(decorate=x,multi):decorate},multi}function line(){var xScale=d3.time.scale(),yScale=d3.scale.linear(),value=identity,keyValue=_index,label=value,decorate=noop,orient="horizontal",dataJoin$$=dataJoin().selector("g.annotation").element("g").attr("class","annotation"),line=function(selection){selection.each(function(data,selectionIndex){
// the value scale which the annotation 'value' relates to, the crossScale
// is the other. Which is which depends on the orienation!
var valueScale,crossScale,translation,lineProperty,handleOne,handleTwo,textAttributes={x:-5,y:-5};switch(orient){case"horizontal":translation=function(a,b){return"translate("+a+", "+b+")"},lineProperty="x2",crossScale=xScale,valueScale=yScale,handleOne="left-handle",handleTwo="right-handle";break;case"vertical":translation=function(a,b){return"translate("+b+", "+a+")"},lineProperty="y2",crossScale=yScale,valueScale=xScale,textAttributes.transform="rotate(-90)",handleOne="bottom-handle",handleTwo="top-handle";break;default:throw new Error("Invalid orientation")}var scaleRange=range(crossScale),
// the transform that sets the 'origin' of the annotation
containerTransform=function(d){var transform=valueScale(value(d));return translation(scaleRange[0],transform)},scaleWidth=scaleRange[1]-scaleRange[0],container=d3.select(this),g=dataJoin$$(container,data),enter=g.enter().attr("transform",containerTransform);enter.append("line").attr(lineProperty,scaleWidth),
// create containers at each end of the annotation
enter.append("g").classed(handleOne,!0),enter.append("g").classed(handleTwo,!0).attr("transform",translation(scaleWidth,0)).append("text").attr(textAttributes),
// Update
...
g.select("line").attr(lineProperty,scaleWidth),g.select("g."+handleTwo).attr("transform",translation(scaleWidth,0)),
// Update the text label
g.select("text").text(label),decorate(g,data,selectionIndex)})};return line.xScale=function(x){return arguments.length?(xScale=x,line):xScale},line.yScale=function(x){return arguments.length?(yScale=x,line):yScale},line.value=function(x){return arguments.length?(value=d3.functor(x),line):value},line.keyValue=function(x){return arguments.length?(keyValue=d3.functor(x),line):keyValue},line.label=function(x){return arguments.length?(label=d3.functor(x),line):label},line.decorate=function(x){return arguments.length?(decorate=x,line):decorate},line.orient=function(x){return arguments.length?(orient=x,line):orient},line}function xyBase(){function base(){}var xScale=d3.time.scale(),yScale=d3.scale.linear(),y0Value=d3.functor(0),x0Value=d3.functor(0),xValue=function(d,i){return d.date},yValue=function(d,i){return d.close};return base.x0=function(d,i){return xScale(x0Value(d,i))},base.y0=function(d,i){return yScale(y0Value(d,i))},base.x=base.x1=function(d,i){return xScale(xValue(d,i))},base.y=base.y1=function(d,i){return yScale(yValue(d,i))},base.defined=function(d,i){return null!=x0Value(d,i)&&null!=y0Value(d,i)&&null!=xValue(d,i)&&null!=yValue(d,i)},base.xScale=function(x){return arguments.length?(xScale=x,base):xScale},base.yScale=function(x){return arguments.length?(yScale=x,base):yScale},base.x0Value=function(x){return arguments.length?(x0Value=d3.functor(x),base):x0Value},base.y0Value=function(x){return arguments.length?(y0Value=d3.functor(x),base):y0Value},base.xValue=base.x1Value=function(x){return arguments.length?(xValue=d3.functor(x),base):xValue},base.yValue=base.y1Value=function(x){return arguments.length?(yValue=d3.functor(x),base):yValue},base}function point(){var decorate=noop,symbol=d3.svg.symbol(),base=xyBase(),dataJoin$$=dataJoin().selector("g.point").element("g").attr("class","point"),containerTransform=function(d,i){return"translate("+base.x(d,i)+", "+base.y(d,i)+")"},point=function(selection){selection.each(function(data,index){var filteredData=data.filter(base.defined),g=dataJoin$$(this,filteredData);g.enter().attr("transform",containerTransform).append("path"),g.attr("transform",containerTransform).select("path").attr("d",symbol),decorate(g,data,index)})};return point.decorate=function(x){return arguments.length?(decorate=x,point):decorate},d3.rebind(point,base,"xScale","xValue","yScale","yValue"),d3.rebind(point,dataJoin$$,"key"),d3.rebind(point,symbol,"size","type"),point}function crosshair(){
...
// these annotations.
function identityScale(scale){return d3.scale.identity().range(range(scale))}function mouseenter(){var mouse=d3.mouse(this),container=d3.select(this),snapped=snap.apply(this,mouse),data=container.datum();data.push(snapped),container.call(crosshair),event.trackingstart.apply(this,arguments)}function mousemove(){var mouse=d3.mouse(this),container=d3.select(this),snapped=snap.apply(this,mouse),data=container.datum();data[data.length-1]=snapped,container.call(crosshair),event.trackingmove.apply(this,arguments)}function mouseleave(){var container=d3.select(this),data=container.datum();data.pop(),container.call(crosshair),event.trackingend.apply(this,arguments)}var event=d3.dispatch("trackingstart","trackingmove","trackingend"),xScale=d3.time.scale(),yScale=d3.scale.linear(),snap=function(_x,_y){return noSnap(xScale,yScale)(_x,_y)},decorate=noop,x=function(d){return d.xInDomainUnits?xScale(d.x):d.x},y=function(d){return d.yInDomainUnits?yScale(d.y):d.y},dataJoin$$=dataJoin().children(!0).selector("g.crosshair").element("g").attr("class","crosshair"),pointSeries=point().xValue(x).yValue(y),horizontalLine=line().value(y).label(function(d){return d.y}),verticalLine=line().orient("vertical").value(x).label(function(d){return d.x}),crosshair=function(selection){selection.each(function(data,index){var container=d3.select(this).style("pointer-events","all").on("mouseenter.crosshair",mouseenter).on("mousemove.crosshair",mousemove).on("mouseleave.crosshair",mouseleave),overlay=container.selectAll("rect").data([data]);overlay.enter().append("rect").style("visibility","hidden"),container.select("rect").attr("x",range(xScale)[0]).attr("y",range(yScale)[1]).attr("width",range(xScale)[1]).attr("height",range(yScale)[0]);var crosshairElement=dataJoin$$(container,data);crosshairElement.enter().style("pointer-events","none");var multi=_multi().series([horizontalLine,verticalLine,pointSeries]).xScale(identityScale(xScale)).yScale(identityScale(yScale)).mapping(function(){return[this]});crosshairElement.call(multi),decorate(crosshairElement,data,index)})};return crosshair.xScale=function(_x){return arguments.length?(xScale=_x,crosshair):xScale},crosshair.yScale=function(_x){return arguments.length?(yScale=_x,crosshair):yScale},crosshair.snap=function(_x){return arguments.length?(snap=_x,crosshair):snap},crosshair.decorate=function(_x){return arguments.length?(decorate=_x,crosshair):decorate},d3.rebind(crosshair,event,"on"),rebind(crosshair,horizontalLine,{yLabel:"label"}),rebind(crosshair,verticalLine,{xLabel:"label"}),crosshair}
// Renders an error bar series as an SVG path based on the given array of datapoints.
function errorBar(){var x=function(d,i){return d.x},y=function(d,i){return d.y},errorHigh=function(d,i){return d.errorHigh},errorLow=function(d,i){return d.errorLow},orient="vertical",barWidth=d3.functor(5),errorBar=function(data){return data.map(function(d,i){var halfWidth=barWidth(d,i)/2,errorTotal=errorHigh(d,i)-errorLow(d,i),yBottom=y(d,i)-errorLow(d,i),yTop=errorHigh(d,i)-y(d,i),xBottom=x(d,i)-errorLow(d,i),xTop=errorHigh(d,i)-x(d,i),errorVertical="",errorHorizontal="";if("vertical"===orient){var horizontalBar="h"+-halfWidth+"h"+2*halfWidth+"h"+-halfWidth,verticalToHigh="v"+-errorTotal;errorVertical="M0,"+yBottom+horizontalBar+verticalToHigh+horizontalBar+"M0,"+yTop}else{var verticalBar="v"+-halfWidth+"v"+2*halfWidth+"v"+-halfWidth,horizontalToHigh="h"+-errorTotal;errorHorizontal="M"+xBottom+",0"+verticalBar+horizontalToHigh+verticalBar+"M"+xTop+",0"}return errorVertical+errorHorizontal}).join("")};return errorBar.x=function(_x){return arguments.length?(x=d3.functor(_x),errorBar):x},errorBar.y=function(_x){return arguments.length?(y=d3.functor(_x),errorBar):y},errorBar.errorHigh=function(_x){return arguments.length?(errorHigh=d3.functor(_x),errorBar):errorHigh},errorBar.errorLow=function(_x){return arguments.length?(errorLow=d3.functor(_x),errorBar):errorLow},errorBar.barWidth=function(_x){return arguments.length?(barWidth=d3.functor(_x),errorBar):barWidth},errorBar.orient=function(_x){return arguments.length?(orient=_x,errorBar):orient},errorBar}
// Renders an OHLC as an SVG path based on the given array of datapoints. Each
// OHLC has a fixed width, whilst the x, open, high, low and close positions are
// obtained from each point via the supplied accessor functions.
function ohlc(){var x=function(d,i){return d.date},open=function(d,i){return d.open},high=function(d,i){return d.high},low=function(d,i){return d.low},close=function(d,i){return d.close},width=d3.functor(3),ohlc=function(data){return data.map(function(d,i){var xValue=x(d,i),yOpen=open(d,i),yHigh=high(d,i),yLow=low(d,i),yClose=close(d,i),halfWidth=width(d,i)/2,moveToLow="M"+xValue+","+yLow,verticalToHigh="V"+yHigh,openTick="M"+xValue+","+yOpen+"h"+-halfWidth,closeTick="M"+xValue+","+yClose+"h"+halfWidth;return moveToLow+verticalToHigh+openTick+closeTick}).join("")};return ohlc.x=function(_x){return arguments.length?(x=d3.functor(_x),ohlc):x},ohlc.open=function(_x){return arguments.length?(open=d3.functor(_x),ohlc):open},ohlc.high=function(_x){return arguments.length?(high=d3.functor(_x),ohlc):high},ohlc.low=function(_x){return arguments.length?(low=d3.functor(_x),ohlc):low},ohlc.close=function(_x){return arguments.length?(close=d3.functor(_x),ohlc):close},ohlc.width=function(_x){return arguments.length?(width=d3.functor(_x),ohlc):width},ohlc}
...
// obtained from each point via the supplied accessor functions.
function candlestick(){var x=function(d,i){return d.date},open=function(d,i){return d.open},high=function(d,i){return d.high},low=function(d,i){return d.low},close=function(d,i){return d.close},width=d3.functor(3),candlestick=function(data){return data.map(function(d,i){var xValue=x(d,i),yOpen=open(d,i),yHigh=high(d,i),yLow=low(d,i),yClose=close(d,i),barWidth=width(d,i),body="M"+(xValue-barWidth/2)+","+yOpen+
// Draw the width
"h"+barWidth+
...
"V"+yOpen+
// Close the path
"z",highWick="M"+xValue+","+Math.min(yClose,yOpen)+"V"+yHigh,lowWick="M"+xValue+","+Math.max(yClose,yOpen)+"V"+yLow;return body+highWick+lowWick}).join("")};return candlestick.x=function(_x){return arguments.length?(x=d3.functor(_x),candlestick):x},candlestick.open=function(_x){return arguments.length?(open=d3.functor(_x),candlestick):open},candlestick.high=function(_x){return arguments.length?(high=d3.functor(_x),candlestick):high},candlestick.low=function(_x){return arguments.length?(low=d3.functor(_x),candlestick):low},candlestick.close=function(_x){return arguments.length?(close=d3.functor(_x),candlestick):close},candlestick.width=function(_x){return arguments.length?(width=d3.functor(_x),candlestick):width},candlestick}
// Renders a bar series as an SVG path based on the given array of datapoints. Each
// bar has a fixed width, whilst the x, y and height are obtained from each data
// point via the supplied accessor functions.
function bar(){var x=function(d,i){return d.x},y=function(d,i){return d.y},horizontalAlign="center",verticalAlign="center",height=function(d,i){return d.height},width=d3.functor(3),bar=function(data,index){return data.map(function(d,i){var horizontalOffset,xValue=x.call(this,d,index||i),yValue=y.call(this,d,index||i),barHeight=height.call(this,d,index||i),barWidth=width.call(this,d,index||i);switch(horizontalAlign){case"left":horizontalOffset=barWidth;break;case"right":horizontalOffset=0;break;case"center":horizontalOffset=barWidth/2;break;default:throw new Error("Invalid horizontal alignment "+horizontalAlign)}var verticalOffset;switch(verticalAlign){case"bottom":verticalOffset=-barHeight;break;case"top":verticalOffset=0;break;case"center":verticalOffset=barHeight/2;break;default:throw new Error("Invalid vertical alignment "+verticalAlign)}
...
"h"+-barWidth+
// Close the path
"z";return body},this).join("")};return bar.x=function(_x){return arguments.length?(x=d3.functor(_x),bar):x},bar.y=function(_x){return arguments.length?(y=d3.functor(_x),bar):y},bar.width=function(_x){return arguments.length?(width=d3.functor(_x),bar):width},bar.horizontalAlign=function(_x){return arguments.length?(horizontalAlign=_x,bar):horizontalAlign},bar.height=function(_x){return arguments.length?(height=d3.functor(_x),bar):height},bar.verticalAlign=function(_x){return arguments.length?(verticalAlign=_x,bar):verticalAlign},bar}
...
// the bound data
function containerTranslate(s,trans){return function(d){return trans(s(d),0)}}function translate(x,y){return isVertical()?"translate("+y+", "+x+")":"translate("+x+", "+y+")"}function pathTranspose(arr){return isVertical()?arr.map(function(d){return[d[1],d[0]]}):arr}function isVertical(){return"left"===orient||"right"===orient}function tryApply(fn,defaultVal){return scale[fn]?scale[fn].apply(scale,tickArguments):defaultVal}var scale=d3.scale.identity(),decorate=noop,orient="bottom",tickArguments=[10],tickValues=null,tickFormat=null,outerTickSize=6,innerTickSize=6,tickPadding=3,svgDomainLine=d3.svg.line(),dataJoin$$=dataJoin().selector("g.tick").element("g").key(identity).attr("class","tick"),domainPathDataJoin=dataJoin().selector("path.domain").element("path").attr("class","domain"),axis=function(selection){selection.each(function(data,index){
// Stash a snapshot of the new scale, and retrieve the old snapshot.
var scaleOld=this.__chart__||scale;this.__chart__=scale.copy();var ticksArray=null==tickValues?tryApply("ticks",scale.domain()):tickValues,tickFormatter=null==tickFormat?tryApply("tickFormat",identity):tickFormat,sign="bottom"===orient||"right"===orient?1:-1,container=d3.select(this),range$$=range(scale),domainPathData=pathTranspose([[range$$[0],sign*outerTickSize],[range$$[0],0],[range$$[1],0],[range$$[1],sign*outerTickSize]]),domainLine=domainPathDataJoin(container,[data]);domainLine.attr("d",svgDomainLine(domainPathData)),
...
transform:containerTranslate(isOrdinal(scale)?scale:scaleOld,translate)});var g=dataJoin$$(container,ticksArray);
// enter
g.enter().append("path");var labelOffset=sign*(innerTickSize+tickPadding);g.enter().append("text").attr("transform",translate(0,labelOffset)),
// update
g.attr("class","tick orient-"+orient),g.attr("transform",containerTranslate(scale,translate)),g.selectAll("path").attr("d",function(d){return svgDomainLine(pathTranspose([[0,0],[0,sign*innerTickSize]]))}),g.selectAll("text").attr("transform",translate(0,labelOffset)).text(tickFormatter),
// exit - for non ordinal scales, exit by animating the tick to its new location
isOrdinal(scale)||g.exit().attr("transform",containerTranslate(scale,translate)),decorate(g,data,index)})};return axis.scale=function(x){return arguments.length?(scale=x,axis):scale},axis.ticks=function(x){return arguments.length?(tickArguments=arguments,axis):tickArguments},axis.tickValues=function(x){return arguments.length?(tickValues=x,axis):tickValues},axis.tickFormat=function(x){return arguments.length?(tickFormat=x,axis):tickFormat},axis.tickSize=function(x){var n=arguments.length;return n?(innerTickSize=Number(x),outerTickSize=Number(arguments[n-1]),axis):innerTickSize},axis.innerTickSize=function(x){return arguments.length?(innerTickSize=Number(x),axis):innerTickSize},axis.outerTickSize=function(x){return arguments.length?(outerTickSize=Number(x),axis):outerTickSize},axis.tickPadding=function(x){return arguments.length?(tickPadding=x,axis):tickPadding},axis.orient=function(x){return arguments.length?(orient=x,axis):orient},axis.decorate=function(x){return arguments.length?(decorate=x,axis):decorate},axis}function errorBase(){function base(){}var xScale=d3.time.scale(),yScale=d3.scale.linear(),errorHigh=d3.functor(0),errorLow=d3.functor(0),xValue=function(d,i){return d.date},yValue=function(d,i){return d.close},orient="vertical",barWidth=d3.functor(5);return base.width=function(data,orientation){return barWidth("vertical"===orientation?data.map(function(d,i){return xScale(xValue(d,i))}):data.map(function(d,i){return yScale(yValue(d,i))}))},base.values=function(d,i){return"vertical"===orient?{x:xScale(xValue(d,i)),y:yScale(yValue(d,i)),errorHigh:yScale(errorHigh(d,i)),errorLow:yScale(errorLow(d,i))}:{x:xScale(xValue(d,i)),y:yScale(yValue(d,i)),errorHigh:xScale(errorHigh(d,i)),errorLow:xScale(errorLow(d,i))}},base.defined=function(d,i){return null!=errorLow(d,i)&&null!=errorHigh(d,i)&&null!=xValue(d,i)&&null!=yValue(d,i)},base.orient=function(x){return arguments.length?(orient=x,base):orient},base.xScale=function(x){return arguments.length?(xScale=x,base):xScale},base.yScale=function(x){return arguments.length?(yScale=x,base):yScale},base.errorLow=function(x){return arguments.length?(errorLow=d3.functor(x),base):errorLow},base.errorHigh=function(x){return arguments.length?(errorHigh=d3.functor(x),base):errorHigh},base.xValue=function(x){return arguments.length?(xValue=d3.functor(x),base):xValue},base.yValue=function(x){return arguments.length?(yValue=d3.functor(x),base):yValue},base.barWidth=function(x){return arguments.length?(barWidth=d3.functor(x),base):barWidth},base}function _errorBar(){var decorate=noop,barWidth=5,orient="vertical",base=errorBase(),dataJoin$$=dataJoin().selector("g.errorBar").element("g").attr("class","errorBar"),errorBar$$=function(selection){base.orient(errorBar$$.orient()),selection.each(function(data,index){var filteredData=data.filter(base.defined),g=dataJoin$$(this,filteredData);g.enter().append("path");var pathGenerator=errorBar().orient(errorBar$$.orient()).barWidth(base.width(filteredData));g.each(function(d,i){var values=base.values(d,i),gErrorBar=d3.select(this).attr("transform","translate("+values.x+", "+values.y+")");pathGenerator.x(values.x).errorHigh(values.errorHigh).errorLow(values.errorLow).y(values.y),gErrorBar.select("path").attr("d",pathGenerator([d])).attr("stroke","black")}),decorate(g,data,index)})};return errorBar$$.barWidth=function(x){return arguments.length?(barWidth=x,errorBar$$):barWidth},errorBar$$.orient=function(x){return arguments.length?(orient=x,errorBar$$):orient},errorBar$$.decorate=function(x){return arguments.length?(decorate=x,errorBar$$):decorate},d3.rebind(errorBar$$,dataJoin$$,"key"),rebindAll(errorBar$$,base),errorBar$$}function ohlcBase(){function base(){}var xScale=d3.time.scale(),yScale=d3.scale.linear(),xValue=function(d,i){return d.date},yOpenValue=function(d,i){return d.open},yHighValue=function(d,i){return d.high},yLowValue=function(d,i){return d.low},yCloseValue=function(d,i){return d.close},barWidth=fractionalBarWidth(.75),xValueScaled=function(d,i){return xScale(xValue(d,i))};return base.width=function(data){return barWidth(data.map(xValueScaled))},base.defined=function(d,i){return null!=xValue(d,i)&&null!=yOpenValue(d,i)&&null!=yLowValue(d,i)&&null!=yHighValue(d,i)&&null!=yCloseValue(d,i)},base.values=function(d,i){var yCloseRaw=yCloseValue(d,i),yOpenRaw=yOpenValue(d,i),direction="";return yCloseRaw>yOpenRaw?direction="up":yOpenRaw>yCloseRaw&&(direction="down"),{x:xValueScaled(d,i),yOpen:yScale(yOpenRaw),yHigh:yScale(yHighValue(d,i)),yLow:yScale(yLowValue(d,i)),yClose:yScale(yCloseRaw),direction:direction}},base.xScale=function(x){return arguments.length?(xScale=x,base):xScale},base.yScale=function(x){return arguments.length?(yScale=x,base):yScale},base.xValue=function(x){return arguments.length?(xValue=x,base):xValue},base.yOpenValue=function(x){return arguments.length?(yOpenValue=x,base):yOpenValue},base.yHighValue=function(x){return arguments.length?(yHighValue=x,base):yHighValue},base.yLowValue=function(x){return arguments.length?(yLowValue=x,base):yLowValue},base.yValue=base.yCloseValue=function(x){return arguments.length?(yCloseValue=x,base):yCloseValue},base.barWidth=function(x){return arguments.length?(barWidth=d3.functor(x),base):barWidth},base}
...
function barSeries(){function containerTranslation(d,i){return"vertical"===orient?"translate("+base.x1(d,i)+", "+base.y0(d,i)+")":"translate("+base.x0(d,i)+", "+base.y1(d,i)+")"}function barHeight(d,i){return"vertical"===orient?base.y1(d,i)-base.y0(d,i):base.x1(d,i)-base.x0(d,i)}function valueAxisDimension(generator){return"vertical"===orient?generator.height:generator.width}function crossAxisDimension(generator){return"vertical"===orient?generator.width:generator.height}function crossAxisValueFunction(){return"vertical"===orient?base.x:base.y}var decorate=noop,barWidth=fractionalBarWidth(.75),orient="vertical",pathGenerator=bar(),base=xyBase().xValue(function(d,i){return"vertical"===orient?d.date:d.close}).yValue(function(d,i){return"vertical"===orient?d.close:d.date}),dataJoin$$=dataJoin().selector("g.bar").element("g"),bar$$=function(selection){selection.each(function(data,index){if("vertical"!==orient&&"horizontal"!==orient)throw new Error("The bar series does not support an orientation of "+orient);dataJoin$$.attr("class","bar "+orient);var filteredData=data.filter(base.defined);pathGenerator.x(0).y(0).width(0).height(0),"vertical"===orient?pathGenerator.verticalAlign("top"):pathGenerator.horizontalAlign("right");
// set the width of the bars
var width=barWidth(filteredData.map(crossAxisValueFunction()));crossAxisDimension(pathGenerator)(width);var g=dataJoin$$(this,filteredData);
...
// from y0 to y1 (y)
g.enter().attr("transform",containerTranslation).append("path").attr("d",function(d){return pathGenerator([d])}),
// set the bar to its correct height
valueAxisDimension(pathGenerator)(barHeight),g.attr("transform",containerTranslation).select("path").attr("d",function(d){return pathGenerator([d])}),decorate(g,filteredData,index)})};return bar$$.decorate=function(x){return arguments.length?(decorate=x,bar$$):decorate},bar$$.barWidth=function(x){return arguments.length?(barWidth=d3.functor(x),bar$$):barWidth},bar$$.orient=function(x){return arguments.length?(orient=x,bar$$):orient},d3.rebind(bar$$,base,"xScale","xValue","x1Value","x0Value","yScale","yValue","y1Value","y0Value"),d3.rebind(bar$$,dataJoin$$,"key"),bar$$}function groupedBar(){var bar=barSeries(),barWidth=fractionalBarWidth(.75),decorate=noop,xScale=d3.scale.linear(),offsetScale=d3.scale.linear(),values=function(d){return d.values},dataJoin$$=dataJoin().selector("g.stacked").element("g").attr("class","stacked"),x=function(d,i){return xScale(bar.xValue()(d,i))},groupedBar=function(selection){selection.each(function(data){var width=barWidth(values(data[0]).map(x)),subBarWidth=width/(data.length-1);bar.barWidth(subBarWidth);var halfWidth=width/2;offsetScale.domain([0,data.length-1]).range([-halfWidth,halfWidth]);var g=dataJoin$$(this,data);g.enter().append("g"),g.select("g").datum(values).each(function(series,index){var container=d3.select(this),compositeScale=function(_x){return xScale(_x)+offsetScale(index)};bar.xScale(compositeScale),
// adapt the decorate function to give each series the correct index
bar.decorate(function(s,d){decorate(s,d,index)}),container.call(bar)})})};return groupedBar.decorate=function(_x){return arguments.length?(decorate=_x,groupedBar):decorate},groupedBar.xScale=function(_x){return arguments.length?(xScale=_x,groupedBar):xScale},groupedBar.values=function(_x){return arguments.length?(values=_x,groupedBar):values},d3.rebind(groupedBar,bar,"yValue","xValue","yScale"),groupedBar}function _stack(){var series=noop,values=function(d){return d.values},stack=function(selection){selection.each(function(data){var container=d3.select(this),dataJoin$$=dataJoin().selector("g.stacked").element("g").attr("class","stacked"),g=dataJoin$$(container,data);g.enter().append("g"),g.select("g").datum(values).call(series)})};return stack.series=function(x){return arguments.length?(series=x,stack):series},stack.values=function(x){return arguments.length?(values=x,stack):values},stack}function _line(){var decorate=noop,base=xyBase(),lineData=d3.svg.line().defined(base.defined).x(base.x).y(base.y),dataJoin$$=dataJoin().selector("path.line").element("path").attr("class","line"),line=function(selection){selection.each(function(data,index){var path=dataJoin$$(this,[data]);path.attr("d",lineData),decorate(path,data,index)})};return line.decorate=function(x){return arguments.length?(decorate=x,line):decorate},d3.rebind(line,base,"xScale","xValue","yScale","yValue"),d3.rebind(line,dataJoin$$,"key"),d3.rebind(line,lineData,"interpolate","tension"),line}function __line(){var line=_line().yValue(function(d){return d.y0+d.y}),stack=_stack().series(line),stackedLine=function(selection){selection.call(stack)};return rebindAll(stackedLine,line),stackedLine}function _bar(){var bar=barSeries().yValue(function(d){return d.y0+d.y}).y0Value(function(d){return d.y0}),stack=_stack().series(bar),stackedBar=function(selection){selection.call(stack)};return rebindAll(stackedBar,bar),stackedBar}function _area(){var decorate=noop,base=xyBase(),areaData=d3.svg.area().defined(base.defined).x(base.x).y0(base.y0).y1(base.y1),dataJoin$$=dataJoin().selector("path.area").element("path").attr("class","area"),area=function(selection){selection.each(function(data,index){var path=dataJoin$$(this,[data]);path.attr("d",areaData),decorate(path,data,index)})};return area.decorate=function(x){return arguments.length?(decorate=x,area):decorate},d3.rebind(area,base,"xScale","xValue","yScale","yValue","y1Value","y0Value"),d3.rebind(area,dataJoin$$,"key"),d3.rebind(area,areaData,"interpolate","tension"),area}function area(){var area=_area().yValue(function(d){return d.y0+d.y}).y0Value(function(d){return d.y0}),stack=_stack().series(area),stackedArea=function(selection){selection.call(stack)};return rebindAll(stackedArea,area),stackedArea}function _ohlc(drawMethod){function containerTranslation(values){return"translate("+values.x+", "+values.yHigh+")"}function xScaleFromCenter(width,scale){var center=width/2,offset=center-scale*center;return"matrix("+scale+", 0, 0, 1, "+offset+", 0)"}var decorate=noop,base=ohlcBase(),dataJoin$$=dataJoin().selector("g.ohlc").element("g").attr("class","ohlc"),ohlc$$=function(selection){selection.each(function(data,index){var filteredData=data.filter(base.defined),g=dataJoin$$(this,filteredData),barWidth=base.width(filteredData);g.enter().attr("transform",function(d,i){return containerTranslation(base.values(d,i))+xScaleFromCenter(barWidth,1e-6)}).append("path");var pathGenerator=ohlc().width(barWidth);g.each(function(d,i){var values=base.values(d,i),graph=d3.transition(d3.select(this)).attr({"class":"ohlc "+values.direction,transform:function(){return containerTranslation(values)+xScaleFromCenter(barWidth,1)}});pathGenerator.x(d3.functor(0)).open(function(){return values.yOpen-values.yHigh}).high(function(){return values.yHigh-values.yHigh}).low(function(){return values.yLow-values.yHigh}).close(function(){return values.yClose-values.yHigh}),graph.select("path").attr("d",pathGenerator([d]))}),decorate(g,data,index)})};return ohlc$$.decorate=function(x){return arguments.length?(decorate=x,ohlc$$):decorate},d3.rebind(ohlc$$,dataJoin$$,"key"),rebindAll(ohlc$$,base),ohlc$$}function cycle(){var decorate=noop,xScale=d3.scale.linear(),yScale=d3.scale.linear(),xValue=function(d,i){return d.date.getDay()},subScale=d3.scale.linear(),subSeries=_line(),barWidth=fractionalBarWidth(.75),dataJoin$$=dataJoin().selector("g.cycle").element("g").attr("class","cycle"),cycle=function(selection){selection.each(function(data,index){var dataByX=d3.nest().key(xValue).map(data),xValues=Object.keys(dataByX),width=barWidth(xValues.map(xScale)),halfWidth=width/2,g=dataJoin$$(this,xValues);g.each(function(d,i){var graph=d3.select(this);graph.attr("transform","translate("+xScale(d)+", 0)"),(subScale.rangeBands||subScale.range)([-halfWidth,halfWidth]),subSeries.xScale(subScale).yScale(yScale),d3.select(this).datum(dataByX[d]).call(subSeries)}),decorate(g,xValues,index)})};return cycle.decorate=function(x){return arguments.length?(decorate=x,cycle):decorate},cycle.xScale=function(x){return arguments.length?(xScale=x,cycle):xScale},cycle.yScale=function(x){return arguments.length?(yScale=x,cycle):yScale},cycle.xValue=function(x){return arguments.length?(xValue=x,cycle):xValue},cycle.subScale=function(x){return arguments.length?(subScale=x,cycle):subScale},cycle.subSeries=function(x){return arguments.length?(subSeries=x,cycle):subSeries},cycle.barWidth=function(x){return arguments.length?(barWidth=d3.functor(x),cycle):barWidth},d3.rebind(cycle,dataJoin$$,"key"),cycle}function _candlestick(){function containerTranslation(values){return"translate("+values.x+", "+values.yHigh+")"}function xScaleFromCenter(width,scale){var center=width/2,offset=center-scale*center;return"matrix("+scale+", 0, 0, 1, "+offset+", 0)"}var decorate=noop,base=ohlcBase(),dataJoin$$=dataJoin().selector("g.candlestick").element("g").attr("class","candlestick"),candlestick$$=function(selection){selection.each(function(data,index){var filteredData=data.filter(base.defined),g=dataJoin$$(this,filteredData),barWidth=base.width(filteredData);g.enter().attr("transform",function(d,i){return containerTranslation(base.values(d,i))+xScaleFromCenter(barWidth,1e-6)}).append("path");var pathGenerator=candlestick().width(barWidth);g.each(function(d,i){var values=base.values(d,i),graph=d3.transition(d3.select(this)).attr({"class":"candlestick "+values.direction,transform:function(){return containerTranslation(values)+xScaleFromCenter(barWidth,1)}});pathGenerator.x(d3.functor(0)).open(function(){return values.yOpen-values.yHigh}).high(function(){return values.yHigh-values.yHigh}).low(function(){return values.yLow-values.yHigh}).close(function(){return values.yClose-values.yHigh}),graph.select("path").attr("d",pathGenerator([d]))}),decorate(g,data,index)})};return candlestick$$.decorate=function(x){return arguments.length?(decorate=x,candlestick$$):decorate},d3.rebind(candlestick$$,dataJoin$$,"key"),rebindAll(candlestick$$,base),candlestick$$}
...
function _axis(){var axis$$=axis(),baseline=d3.functor(0),decorate=noop,xScale=d3.time.scale(),yScale=d3.scale.linear(),dataJoin$$=dataJoin().selector("g.axis-adapter").element("g").attr({"class":"axis axis-adapter"}),axisAdapter=function(selection){selection.each(function(data,index){var translation,g=dataJoin$$(this,[data]);switch(axisAdapter.orient()){case"top":case"bottom":translation="translate(0,"+yScale(baseline(data))+")",axis$$.scale(xScale);break;case"left":case"right":translation="translate("+xScale(baseline(data))+",0)",axis$$.scale(yScale);break;default:throw new Error("Invalid orientation")}g.enter().attr("transform",translation),g.attr("transform",translation),g.call(axis$$),decorate(g,data,index)})};return axisAdapter.baseline=function(x){return arguments.length?(baseline=d3.functor(x),axisAdapter):baseline},axisAdapter.decorate=function(x){return arguments.length?(decorate=x,axisAdapter):decorate},axisAdapter.xScale=function(x){return arguments.length?(xScale=x,axisAdapter):xScale},axisAdapter.yScale=function(x){return arguments.length?(yScale=x,axisAdapter):yScale},d3.rebind(axisAdapter,axis$$,"orient","ticks","tickValues","tickSize","innerTickSize","outerTickSize","tickPadding","tickFormat")}function _identity(){var identity$$={};return identity$$.distance=function(startDate,endDate){return endDate.getTime()-startDate.getTime()},identity$$.offset=function(startDate,ms){return new Date(startDate.getTime()+ms)},identity$$.clampUp=identity,identity$$.clampDown=identity,identity$$.copy=function(){return identity$$},identity$$}
// obtains the ticks from the given scale, transforming the result to ensure
// it does not include any discontinuities
function tickTransformer(ticks,discontinuityProvider,domain){var clampedTicks=ticks.map(function(tick,index){if(index<ticks.length-1)return discontinuityProvider.clampUp(tick);var clampedTick=discontinuityProvider.clampUp(tick);return clampedTick<domain[1]?clampedTick:discontinuityProvider.clampDown(tick)}),uniqueTicks=clampedTicks.reduce(function(arr,tick){return 0===arr.filter(function(f){return f.getTime()===tick.getTime()}).length&&arr.push(tick),arr},[]);return uniqueTicks}/**
    ...
    */
function dateTimeScale(adaptedScale,discontinuityProvider){function scale(date){var domain=adaptedScale.domain(),range=adaptedScale.range(),totalDomainDistance=discontinuityProvider.distance(domain[0],domain[1]),distanceToX=discontinuityProvider.distance(domain[0],date),ratioToX=distanceToX/totalDomainDistance,scaledByRange=ratioToX*(range[1]-range[0])+range[0];return scaledByRange}return arguments.length||(adaptedScale=d3.time.scale(),discontinuityProvider=_identity()),scale.invert=function(x){var domain=adaptedScale.domain(),range=adaptedScale.range(),ratioToX=(x-range[0])/(range[1]-range[0]),totalDomainDistance=discontinuityProvider.distance(domain[0],domain[1]),distanceToX=ratioToX*totalDomainDistance;return discontinuityProvider.offset(domain[0],distanceToX)},scale.domain=function(x){if(!arguments.length)return adaptedScale.domain();
// clamp the upper and lower domain values to ensure they
// do not fall within a discontinuity
var domainLower=discontinuityProvider.clampUp(x[0]),domainUpper=discontinuityProvider.clampDown(x[1]);return adaptedScale.domain([domainLower,domainUpper]),scale},scale.nice=function(){adaptedScale.nice();var domain=adaptedScale.domain(),domainLower=discontinuityProvider.clampUp(domain[0]),domainUpper=discontinuityProvider.clampDown(domain[1]);return adaptedScale.domain([domainLower,domainUpper]),scale},scale.ticks=function(){var ticks=adaptedScale.ticks.apply(this,arguments);return tickTransformer(ticks,discontinuityProvider,scale.domain())},scale.copy=function(){return dateTimeScale(adaptedScale.copy(),discontinuityProvider.copy())},scale.discontinuityProvider=function(x){return arguments.length?(discontinuityProvider=x,scale):discontinuityProvider},d3.rebind(scale,adaptedScale,"range","rangeRound","interpolate","clamp","tickFormat")}function exportedScale(){return dateTimeScale()}function skipWeekends(){function isWeekend(date){return 0===date.getDay()||6===date.getDay()}var millisPerDay=864e5,millisPerWorkWeek=5*millisPerDay,millisPerWeek=7*millisPerDay,skipWeekends={};
// returns the number of included milliseconds (i.e. those which do not fall)
// within discontinuities, along this scale
return skipWeekends.clampDown=function(date){if(date&&isWeekend(date)){var daysToSubtract=0===date.getDay()?2:1,newDate=d3.time.day.ceil(date);
// then subtract the required number of days
return d3.time.day.offset(newDate,-daysToSubtract)}return date},skipWeekends.clampUp=function(date){if(date&&isWeekend(date)){var daysToAdd=0===date.getDay()?1:2,newDate=d3.time.day.floor(date);
// then add the required number of days
return d3.time.day.offset(newDate,daysToAdd)}return date},skipWeekends.distance=function(startDate,endDate){startDate=skipWeekends.clampUp(startDate),endDate=skipWeekends.clampDown(endDate);
// move the start date to the end of week boundary
var offsetStart=d3.time.saturday.ceil(startDate);if(offsetStart>endDate)return endDate.getTime()-startDate.getTime();var msAdded=offsetStart.getTime()-startDate.getTime(),offsetEnd=d3.time.saturday.ceil(endDate),msRemoved=offsetEnd.getTime()-endDate.getTime(),weeks=(offsetEnd.getTime()-offsetStart.getTime())/millisPerWeek;return weeks*millisPerWorkWeek+msAdded-msRemoved},skipWeekends.offset=function(startDate,ms){var date=isWeekend(startDate)?skipWeekends.clampUp(startDate):startDate,remainingms=ms,endOfWeek=d3.time.saturday.ceil(date);
// if the distance to the boundary is greater than the number of ms
// simply add the ms to the current date
if(remainingms-=endOfWeek.getTime()-date.getTime(),0>remainingms)return new Date(date.getTime()+ms);
// skip the weekend
...
return date=d3.time.day.offset(date,7*completeWeeks),remainingms-=completeWeeks*millisPerWorkWeek,date=new Date(date.getTime()+remainingms)},skipWeekends.copy=function(){return skipWeekends},skipWeekends}function elderRay(){var xScale=d3.time.scale(),yScale=d3.scale.linear(),xValue=function(d){return d.date},root=function(d){return d.elderRay},bullBar=barSeries(),bearBar=barSeries(),bullBarTop=barSeries(),bearBarTop=barSeries(),multi=_multi(),decorate=noop,elderRay=function(selection){function isTop(input,comparison){
// The values share parity and the input is smaller than the comparison
return input*comparison>0&&Math.abs(input)<Math.abs(comparison)}bullBar.xValue(xValue).yValue(function(d,i){return isTop(root(d).bullPower,root(d).bearPower)?void 0:root(d).bullPower}),bearBar.xValue(xValue).yValue(function(d,i){return isTop(root(d).bearPower,root(d).bullPower)?void 0:root(d).bearPower}),bullBarTop.xValue(xValue).yValue(function(d,i){return isTop(root(d).bullPower,root(d).bearPower)?root(d).bullPower:void 0}),bearBarTop.xValue(xValue).yValue(function(d,i){return isTop(root(d).bearPower,root(d).bullPower)?root(d).bearPower:void 0}),multi.xScale(xScale).yScale(yScale).series([bullBar,bearBar,bullBarTop,bearBarTop]).decorate(function(g,data,index){g.enter().attr("class",function(d,i){return"multi "+["bull","bear","bull top","bear top"][i]}),decorate(g,data,index)}),selection.call(multi)};return elderRay.xScale=function(x){return arguments.length?(xScale=x,elderRay):xScale},elderRay.xValue=function(x){return arguments.length?(xValue=x,elderRay):xValue},elderRay.yScale=function(x){return arguments.length?(yScale=x,elderRay):yScale},elderRay.decorate=function(x){return arguments.length?(decorate=x,elderRay):decorate},elderRay}function envelope(){var xScale=d3.time.scale(),yScale=d3.scale.linear(),yValue=function(d,i){return d.close},xValue=function(d,i){return d.date},root=function(d){return d.envelope},decorate=noop,area=_area().y0Value(function(d,i){return root(d).upper}).y1Value(function(d,i){return root(d).lower}),upperLine=_line().yValue(function(d,i){return root(d).upper}),lowerLine=_line().yValue(function(d,i){return root(d).lower}),envelope=function(selection){var multi=_multi().xScale(xScale).yScale(yScale).series([area,upperLine,lowerLine]).decorate(function(g,data,index){g.enter().attr("class",function(d,i){return"multi envelope "+["area","upper","lower"][i]}),decorate(g,data,index)});area.xValue(xValue),upperLine.xValue(xValue),lowerLine.xValue(xValue),selection.call(multi)};return envelope.xScale=function(x){return arguments.length?(xScale=x,envelope):xScale},envelope.yScale=function(x){return arguments.length?(yScale=x,envelope):yScale},envelope.xValue=function(x){return arguments.length?(xValue=x,envelope):xValue},envelope.yValue=function(x){return arguments.length?(yValue=x,envelope):yValue},envelope.root=function(x){return arguments.length?(root=x,envelope):root},envelope.decorate=function(x){return arguments.length?(decorate=x,envelope):decorate},envelope}function forceIndex(){var xScale=d3.time.scale(),yScale=d3.scale.linear(),multiSeries=_multi(),decorate=noop,annotations=line(),forceLine=_line().yValue(function(d,i){return d.force}),force=function(selection){multiSeries.xScale(xScale).yScale(yScale).series([annotations,forceLine]).mapping(function(series){return series===annotations?[0]:this}).decorate(function(g,data,index){g.enter().attr("class",function(d,i){return"multi "+["annotations","indicator"][i]}),decorate(g,data,index)}),selection.call(multiSeries)};return force.xScale=function(x){return arguments.length?(xScale=x,annotations.xScale(x),force):xScale},force.yScale=function(x){return arguments.length?(yScale=x,annotations.yScale(x),force):yScale},force.decorate=function(x){return arguments.length?(decorate=x,force):decorate},d3.rebind(force,forceLine,"yValue","xValue"),force}function stochasticOscillator(){var xScale=d3.time.scale(),yScale=d3.scale.linear(),upperValue=80,lowerValue=20,multi=_multi(),decorate=noop,annotations=line(),dLine=_line().yValue(function(d,i){return d.stochastic.d}),kLine=_line().yValue(function(d,i){return d.stochastic.k}),stochastic=function(selection){multi.xScale(xScale).yScale(yScale).series([annotations,dLine,kLine]).mapping(function(series){return series===annotations?[upperValue,lowerValue]:this}).decorate(function(g,data,index){g.enter().attr("class",function(d,i){return"multi stochastic "+["annotations","stochastic-d","stochastic-k"][i]}),decorate(g,data,index)}),selection.call(multi)};return stochastic.xScale=function(x){return arguments.length?(xScale=x,stochastic):xScale},stochastic.yScale=function(x){return arguments.length?(yScale=x,stochastic):yScale},stochastic.upperValue=function(x){return arguments.length?(upperValue=x,stochastic):upperValue},stochastic.lowerValue=function(x){return arguments.length?(lowerValue=x,stochastic):lowerValue},stochastic.decorate=function(x){return arguments.length?(decorate=x,stochastic):decorate},d3.rebind(stochastic,dLine,"yDValue","xDValue"),d3.rebind(stochastic,kLine,"yKValue","xKValue"),stochastic}function relativeStrengthIndex(){var xScale=d3.time.scale(),yScale=d3.scale.linear(),upperValue=70,lowerValue=30,multiSeries=_multi(),decorate=noop,annotations=line(),rsiLine=_line().yValue(function(d,i){return d.rsi}),rsi=function(selection){multiSeries.xScale(xScale).yScale(yScale).series([rsiLine,annotations]).mapping(function(series){return series===annotations?[upperValue,50,lowerValue]:this}).decorate(function(g,data,index){g.enter().attr("class",function(d,i){return"multi rsi "+["indicator","annotations"][i]}),decorate(g,data,index)}),selection.call(multiSeries)};return rsi.xScale=function(x){return arguments.length?(xScale=x,rsi):xScale},rsi.yScale=function(x){return arguments.length?(yScale=x,rsi):yScale},rsi.upperValue=function(x){return arguments.length?(upperValue=x,rsi):upperValue},rsi.lowerValue=function(x){return arguments.length?(lowerValue=x,rsi):lowerValue},rsi.decorate=function(x){return arguments.length?(decorate=x,rsi):decorate},d3.rebind(rsi,rsiLine,"yValue","xValue"),rsi}function macd(){var xScale=d3.time.scale(),yScale=d3.scale.linear(),xValue=function(d){return d.date},root=function(d){return d.macd},macdLine=_line(),signalLine=_line(),divergenceBar=barSeries(),multiSeries=_multi(),decorate=noop,macd=function(selection){macdLine.xValue(xValue).yValue(function(d,i){return root(d).macd}),signalLine.xValue(xValue).yValue(function(d,i){return root(d).signal}),divergenceBar.xValue(xValue).yValue(function(d,i){return root(d).divergence}),multiSeries.xScale(xScale).yScale(yScale).series([divergenceBar,macdLine,signalLine]).decorate(function(g,data,index){g.enter().attr("class",function(d,i){return"multi "+["macd-divergence","macd","macd-signal"][i]}),decorate(g,data,index)}),selection.call(multiSeries)};return macd.xScale=function(x){return arguments.length?(xScale=x,macd):xScale},macd.xValue=function(x){return arguments.length?(xValue=x,macd):xValue},macd.yScale=function(x){return arguments.length?(yScale=x,macd):yScale},macd.root=function(x){return arguments.length?(root=x,macd):root},macd.decorate=function(x){return arguments.length?(decorate=x,macd):decorate},macd}function bollingerBands(){var xScale=d3.time.scale(),yScale=d3.scale.linear(),yValue=function(d,i){return d.close},xValue=function(d,i){return d.date},root=function(d){return d.bollingerBands},decorate=noop,area=_area().y0Value(function(d,i){return root(d).upper}).y1Value(function(d,i){return root(d).lower}),upperLine=_line().yValue(function(d,i){return root(d).upper}),averageLine=_line().yValue(function(d,i){return root(d).average}),lowerLine=_line().yValue(function(d,i){return root(d).lower}),bollingerBands=function(selection){var multi=_multi().xScale(xScale).yScale(yScale).series([area,upperLine,lowerLine,averageLine]).decorate(function(g,data,index){g.enter().attr("class",function(d,i){return"multi bollinger "+["area","upper","lower","average"][i]}),decorate(g,data,index)});area.xValue(xValue),upperLine.xValue(xValue),averageLine.xValue(xValue),lowerLine.xValue(xValue),selection.call(multi)};return bollingerBands.xScale=function(x){return arguments.length?(xScale=x,bollingerBands):xScale},bollingerBands.yScale=function(x){return arguments.length?(yScale=x,bollingerBands):yScale},bollingerBands.xValue=function(x){return arguments.length?(xValue=x,bollingerBands):xValue},bollingerBands.yValue=function(x){return arguments.length?(yValue=x,bollingerBands):yValue},bollingerBands.root=function(x){return arguments.length?(root=x,bollingerBands):root},bollingerBands.decorate=function(x){return arguments.length?(decorate=x,bollingerBands):decorate},bollingerBands}function calculator(){var undefinedValue=d3.functor(void 0),windowSize=d3.functor(10),accumulator=noop,value=identity,slidingWindow=function(data){var size=windowSize.apply(this,arguments),windowData=data.slice(0,size).map(value);return data.map(function(d,i){
// Treat windowData as FIFO rolling buffer
return size-1>i?undefinedValue(d,i):(i>=size&&(windowData.shift(),windowData.push(value(d,i))),accumulator(windowData))})};return slidingWindow.undefinedValue=function(x){return arguments.length?(undefinedValue=d3.functor(x),slidingWindow):undefinedValue},slidingWindow.windowSize=function(x){return arguments.length?(windowSize=d3.functor(x),slidingWindow):windowSize},slidingWindow.accumulator=function(x){return arguments.length?(accumulator=x,slidingWindow):accumulator},slidingWindow.value=function(x){return arguments.length?(value=x,slidingWindow):value},slidingWindow}
// applies an algorithm to an array, merging the result back into
// the source array using the given merge function.
function merge(){var merge=noop,algorithm=calculator(),mergeCompute=function(data){return d3.zip(data,algorithm(data)).forEach(function(tuple){merge(tuple[0],tuple[1])})};return mergeCompute.algorithm=function(x){return arguments.length?(algorithm=x,mergeCompute):algorithm},mergeCompute.merge=function(x){return arguments.length?(merge=x,mergeCompute):merge},mergeCompute}function exponentialMovingAverage(){var windowSize=9,value=identity,exponentialMovingAverage=function(data){var previous,alpha=2/(windowSize+1),initialAccumulator=0;return data.map(function(d,i){if(windowSize-1>i)return void(initialAccumulator+=value(d,i));if(i===windowSize-1){initialAccumulator+=value(d,i);var initialValue=initialAccumulator/windowSize;return previous=initialValue,initialValue}var nextValue=value(d,i)*alpha+(1-alpha)*previous;return previous=nextValue,nextValue})};return exponentialMovingAverage.windowSize=function(x){return arguments.length?(windowSize=x,exponentialMovingAverage):windowSize},exponentialMovingAverage.value=function(x){return arguments.length?(value=x,exponentialMovingAverage):value},exponentialMovingAverage}function _calculator(){var value=identity,highValue=function(d,i){return d.high},lowValue=function(d,i){return d.low},emaComputer=exponentialMovingAverage().windowSize(13),elderRay=function(data){emaComputer.value(value);var ema=emaComputer(data),indicator=d3.zip(data,ema).map(function(d){return{bullPower:d[1]?highValue(d[0])-d[1]:void 0,bearPower:d[1]?lowValue(d[0])-d[1]:void 0}});return indicator};return elderRay.value=function(x){return arguments.length?(value=x,elderRay):value},elderRay.highValue=function(x){return arguments.length?(highValue=x,elderRay):highValue},elderRay.lowValue=function(x){return arguments.length?(lowValue=x,elderRay):highValue},rebind(elderRay,emaComputer,{period:"windowSize"}),elderRay}function _elderRay(){var elderRayAlgorithm=_calculator().value(function(d){return d.close}),mergedAlgorithm=merge().algorithm(elderRayAlgorithm).merge(function(datum,indicator){datum.elderRay=indicator}),elderRay=function(data){return mergedAlgorithm(data)};return d3.rebind(elderRay,mergedAlgorithm,"merge"),d3.rebind(elderRay,elderRayAlgorithm,"highValue","lowValue","period","value"),elderRay}
...
function undefinedInputAdapter(){function undefinedArrayOfLength(length){return Array.apply(null,new Array(length)).map(undefinedValue)}var algorithm=calculator().accumulator(d3.mean),undefinedValue=d3.functor(void 0),defined=function(value){return null==algorithm.value()(value)},undefinedInputAdapter=function(data){for(var undefinedCount=0;defined(data[undefinedCount])&&undefinedCount<data.length;)undefinedCount++;var nonUndefinedData=data.slice(undefinedCount);return undefinedArrayOfLength(undefinedCount).concat(algorithm(nonUndefinedData))};return undefinedInputAdapter.algorithm=function(x){return arguments.length?(algorithm=x,undefinedInputAdapter):algorithm},undefinedInputAdapter.undefinedValue=function(x){return arguments.length?(undefinedValue=d3.functor(x),undefinedInputAdapter):undefinedValue},undefinedInputAdapter.defined=function(x){return arguments.length?(defined=x,undefinedInputAdapter):defined},undefinedInputAdapter}function envelopeCalculator(){var factor=.1,value=identity,envelope=function(data){return data.map(function(s){return{lower:value(s)*(1-factor),upper:value(s)*(1+factor)}})};return envelope.factor=function(x){return arguments.length?(factor=x,envelope):factor},envelope.value=function(x){return arguments.length?(value=d3.functor(x),envelope):value},envelope}function _envelope(){var envelopeAlgorithm=envelopeCalculator(),adaptedEnvelope=undefinedInputAdapter().undefinedValue({lower:void 0,upper:void 0}).algorithm(envelopeAlgorithm),mergedAlgorithm=merge().algorithm(adaptedEnvelope).merge(function(datum,env){datum.envelope=env}),envelope=function(data){return mergedAlgorithm(data)};return envelope.root=function(d){return d.envelope},d3.rebind(envelope,mergedAlgorithm,"merge"),d3.rebind(envelope,envelopeAlgorithm,"value","factor"),envelope}function __calculator(){var volumeValue=function(d,i){return d.volume},closeValue=function(d,i){return d.close},slidingWindow=calculator().windowSize(2).accumulator(function(values){return(closeValue(values[1])-closeValue(values[0]))*volumeValue(values[1])}),force=function(data){return slidingWindow(data)};return force.volumeValue=function(x){return arguments.length?(volumeValue=x,force):volumeValue},force.closeValue=function(x){return arguments.length?(closeValue=x,force):closeValue},d3.rebind(force,slidingWindow,"windowSize"),force}function _forceIndex(){var force=__calculator(),mergedAlgorithm=merge().algorithm(force).merge(function(datum,indicator){datum.force=indicator}),forceIndex=function(data){return mergedAlgorithm(data)};return d3.rebind(forceIndex,mergedAlgorithm,"merge"),d3.rebind(forceIndex,force,"windowSize","volumeValue","closeValue"),forceIndex}function ___calculator(){var closeValue=function(d,i){return d.close},highValue=function(d,i){return d.high},lowValue=function(d,i){return d.low},kWindow=calculator().windowSize(5).accumulator(function(values){var maxHigh=d3.max(values,highValue),minLow=d3.min(values,lowValue);return 100*(closeValue(values[values.length-1])-minLow)/(maxHigh-minLow)}),dWindow=calculator().windowSize(3).accumulator(function(values){return void 0===values[0]?void 0:d3.mean(values)}),stochastic=function(data){var kValues=kWindow(data),dValues=dWindow(kValues);return kValues.map(function(k,i){var d=dValues[i];return{k:k,d:d}})};return stochastic.closeValue=function(x){return arguments.length?(closeValue=x,stochastic):closeValue},stochastic.highValue=function(x){return arguments.length?(highValue=x,stochastic):highValue},stochastic.lowValue=function(x){return arguments.length?(lowValue=x,stochastic):highValue},rebind(stochastic,kWindow,{kWindowSize:"windowSize"}),rebind(stochastic,dWindow,{dWindowSize:"windowSize"}),stochastic}function _stochasticOscillator(){var stoc=___calculator(),mergedAlgorithm=merge().algorithm(stoc).merge(function(datum,indicator){datum.stochastic=indicator}),stochasticOscillator=function(data){return mergedAlgorithm(data)};return d3.rebind(stochasticOscillator,mergedAlgorithm,"merge"),d3.rebind(stochasticOscillator,stoc,"kWindowSize","dWindowSize","lowValue","closeValue","highValue"),stochasticOscillator}function ____calculator(){var openValue=function(d,i){return d.open},closeValue=function(d,i){return d.close},averageAccumulator=function(values){for(var alpha=1/values.length,result=values[0],i=1,l=values.length;l>i;i++)result=alpha*values[i]+(1-alpha)*result;return result},slidingWindow=calculator().windowSize(14).accumulator(function(values){for(var downCloses=[],upCloses=[],i=0,l=values.length;l>i;i++){var value=values[i],open=openValue(value),close=closeValue(value);downCloses.push(open>close?open-close:0),upCloses.push(close>open?close-open:0)}var downClosesAvg=averageAccumulator(downCloses);if(0===downClosesAvg)return 100;var rs=averageAccumulator(upCloses)/downClosesAvg;return 100-100/(1+rs)}),rsi=function(data){return slidingWindow(data)};return rsi.openValue=function(x){return arguments.length?(openValue=x,rsi):openValue},rsi.closeValue=function(x){return arguments.length?(closeValue=x,rsi):closeValue},d3.rebind(rsi,slidingWindow,"windowSize"),rsi}function _relativeStrengthIndex(){var rsi=____calculator(),mergedAlgorithm=merge().algorithm(rsi).merge(function(datum,indicator){datum.rsi=indicator}),relativeStrengthIndex=function(data){return mergedAlgorithm(data)};return d3.rebind(relativeStrengthIndex,mergedAlgorithm,"merge"),d3.rebind(relativeStrengthIndex,rsi,"windowSize","openValue","closeValue"),relativeStrengthIndex}function movingAverage(){var ma=calculator().accumulator(d3.mean).value(function(d){return d.close}),mergedAlgorithm=merge().algorithm(ma).merge(function(datum,indicator){datum.movingAverage=indicator}),movingAverage=function(data){return mergedAlgorithm(data)};return d3.rebind(movingAverage,mergedAlgorithm,"merge"),d3.rebind(movingAverage,ma,"windowSize","undefinedValue","value"),movingAverage}function _____calculator(){var value=identity,fastEMA=exponentialMovingAverage().windowSize(12),slowEMA=exponentialMovingAverage().windowSize(29),signalEMA=exponentialMovingAverage().windowSize(9),adaptedSignalEMA=undefinedInputAdapter().algorithm(signalEMA),macd=function(data){fastEMA.value(value),slowEMA.value(value);var diff=d3.zip(fastEMA(data),slowEMA(data)).map(function(d){return void 0!==d[0]&&void 0!==d[1]?d[0]-d[1]:void 0}),averageDiff=adaptedSignalEMA(diff);return d3.zip(diff,averageDiff).map(function(d){return{macd:d[0],signal:d[1],divergence:void 0!==d[0]&&void 0!==d[1]?d[0]-d[1]:void 0}})};return macd.value=function(x){return arguments.length?(value=x,macd):value},rebind(macd,fastEMA,{fastPeriod:"windowSize"}),rebind(macd,slowEMA,{slowPeriod:"windowSize"}),rebind(macd,signalEMA,{signalPeriod:"windowSize"}),macd}function _macd(){var macdAlgorithm=_____calculator().value(function(d){return d.close}),mergedAlgorithm=merge().algorithm(macdAlgorithm).merge(function(datum,indicator){datum.macd=indicator}),macd=function(data){return mergedAlgorithm(data)};return d3.rebind(macd,mergedAlgorithm,"merge"),d3.rebind(macd,macdAlgorithm,"fastPeriod","slowPeriod","signalPeriod","value"),macd}function _exponentialMovingAverage(){var ema=exponentialMovingAverage().value(function(d){return d.close}),mergedAlgorithm=merge().algorithm(ema).merge(function(datum,indicator){datum.exponentialMovingAverage=indicator}),exponentialMovingAverage$$=function(data){return mergedAlgorithm(data)};return d3.rebind(exponentialMovingAverage$$,mergedAlgorithm,"merge"),d3.rebind(exponentialMovingAverage$$,ema,"windowSize","value"),exponentialMovingAverage$$}function percentageChange(){var baseIndex=d3.functor(0),value=identity,percentageChange=function(data){if(0===data.length)return[];var baseValue=value(data[baseIndex(data)]);return data.map(function(d,i){return(value(d,i)-baseValue)/baseValue})};return percentageChange.baseIndex=function(x){return arguments.length?(baseIndex=d3.functor(x),percentageChange):baseIndex},percentageChange.value=function(x){return arguments.length?(value=x,percentageChange):value},percentageChange}function _bollingerBands(){var multiplier=2,slidingWindow=calculator().undefinedValue({upper:void 0,average:void 0,lower:void 0}).accumulator(function(values){var avg=d3.mean(values),stdDev=d3.deviation(values);return{upper:avg+multiplier*stdDev,average:avg,lower:avg-multiplier*stdDev}}),bollingerBands=function(data){return slidingWindow(data)};return bollingerBands.multiplier=function(x){return arguments.length?(multiplier=x,bollingerBands):multiplier},d3.rebind(bollingerBands,slidingWindow,"windowSize","value"),bollingerBands}function __bollingerBands(){var bollingerAlgorithm=_bollingerBands().value(function(d){return d.close}),mergedAlgorithm=merge().algorithm(bollingerAlgorithm).merge(function(datum,indicator){datum.bollingerBands=indicator}),bollingerBands=function(data){return mergedAlgorithm(data)};return bollingerBands.root=function(d){return d.bollingerBands},d3.rebind(bollingerBands,mergedAlgorithm,"merge"),d3.rebind(bollingerBands,bollingerAlgorithm,"windowSize","value","multiplier"),bollingerBands}function modeMedian(){function getBuckets(data){for(var numberOfBuckets=Math.ceil((data.length-2)/bucketSize),trimmedData=data.slice(1,data.length-1),buckets=[],i=0;numberOfBuckets>i;i++)buckets.push(trimmedData.slice(i*bucketSize,(i+1)*bucketSize));return buckets}var bucketSize=10,value=identity,modeMedian=function(data){if(bucketSize>data.length)return data;var minMax=d3.extent(data),buckets=getBuckets(data),subsampledData=buckets.map(function(bucket,i){for(var mostFrequent,mostFrequentIndex,frequencies={},singleMostFrequent=!0,j=0;j<bucket.length;j++){var item=value(bucket[j]);if(item===minMax[0]||item===minMax[1])return bucket[j];void 0===frequencies[item]&&(frequencies[item]=0),frequencies[item]++,frequencies[item]>frequencies[mostFrequent]||void 0===mostFrequent?(mostFrequent=item,mostFrequentIndex=j,singleMostFrequent=!0):frequencies[item]===frequencies[mostFrequent]&&(singleMostFrequent=!1)}return singleMostFrequent?bucket[mostFrequentIndex]:bucket[Math.floor(bucket.length/2)]});
// First and last data points are their own buckets.
return[].concat(data[0],subsampledData,data[data.length-1])};return modeMedian.bucketSize=function(x){return arguments.length?(bucketSize=x,modeMedian):bucketSize},modeMedian.value=function(x){return arguments.length?(value=x,modeMedian):value},modeMedian}
...
function spread(){function verticalSpread(data){var series=Object.keys(data[0]).filter(function(key){return key!==xValueKey}).map(function(key){var values=data.filter(function(row){return row[key]}).map(function(row){return{x:row[xValueKey],y:yValue(row,key)}});return{key:key,values:values}});return series}function horizontalSpread(data){var series=data.map(function(row){var keys=Object.keys(row).filter(function(d){return d!==xValueKey});return{key:row[xValueKey],values:keys.map(function(key){return{x:key,y:yValue(row,key)}})}});return series}var xValueKey="",orient="vertical",yValue=function(row,key){
// D3 CSV returns all values as strings, this converts them to numbers
// by default.
return Number(row[key])},spread=function(data){return"vertical"===orient?verticalSpread(data):horizontalSpread(data)};return spread.xValueKey=function(x){return arguments.length?(xValueKey=x,spread):xValueKey},spread.yValue=function(x){return arguments.length?(yValue=x,spread):yValue},spread.orient=function(x){return arguments.length?(orient=x,spread):orient},spread}function walk(){var period=1,steps=20,mu=.1,sigma=.1,walk=function(initial){var increment,step,randomNormal=d3.random.normal(),timeStep=period/steps,increments=new Array(steps+1);
// Compute step increments for the discretized GBM model.
for(step=1;step<increments.length;step+=1)increment=randomNormal(),increment*=Math.sqrt(timeStep),increment*=sigma,increment+=(mu-sigma*sigma/2)*timeStep,increments[step]=Math.exp(increment);for(
// Return the cumulative product of increments from initial value.
increments[0]=initial,step=1;step<increments.length;step+=1)increments[step]=increments[step-1]*increments[step];return increments};return walk.period=function(x){return arguments.length?(period=x,walk):period},walk.steps=function(x){return arguments.length?(steps=x,walk):steps},walk.mu=function(x){return arguments.length?(mu=x,walk):mu},walk.sigma=function(x){return arguments.length?(sigma=x,walk):sigma},walk}function financial(){var mu=.1,sigma=.1,startPrice=100,startVolume=1e5,startDate=new Date,stepsPerDay=50,volumeNoiseFactor=.3,filter=function(date){return!(0===date.getDay()||6===date.getDay())},calculateOHLC=function(days,prices,volumes){for(var daySteps,ohlcv=[],currentStep=0,currentIntraStep=0;ohlcv.length<days;)daySteps=prices.slice(currentIntraStep,currentIntraStep+stepsPerDay),ohlcv.push({date:new Date(startDate.getTime()),open:daySteps[0],high:Math.max.apply({},daySteps),low:Math.min.apply({},daySteps),close:daySteps[stepsPerDay-1],volume:volumes[currentStep]}),currentIntraStep+=stepsPerDay,currentStep+=1,startDate.setUTCDate(startDate.getUTCDate()+1);return ohlcv},gen=function(days){var toDate=new Date(startDate.getTime());toDate.setUTCDate(startDate.getUTCDate()+days);var millisecondsPerYear=315569e5,years=(toDate.getTime()-startDate.getTime())/millisecondsPerYear,prices=walk().period(years).steps(days*stepsPerDay).mu(mu).sigma(sigma)(startPrice),volumes=walk().period(years).steps(days).mu(0).sigma(sigma)(startVolume);
// Add random noise
// Save the new start values
return volumes=volumes.map(function(vol){var boundedNoiseFactor=Math.min(0,Math.max(volumeNoiseFactor,1)),multiplier=1+boundedNoiseFactor*(1-2*Math.random());return Math.floor(vol*multiplier)}),startPrice=prices[prices.length-1],startVolume=volumes[volumes.length-1],calculateOHLC(days,prices,volumes).filter(function(d){return!filter||filter(d.date)})};return gen.mu=function(x){return arguments.length?(mu=x,gen):mu},gen.sigma=function(x){return arguments.length?(sigma=x,gen):sigma},gen.startPrice=function(x){return arguments.length?(startPrice=x,gen):startPrice},gen.startVolume=function(x){return arguments.length?(startVolume=x,gen):startVolume},gen.startDate=function(x){return arguments.length?(startDate=x,gen):startDate},gen.stepsPerDay=function(x){return arguments.length?(stepsPerDay=x,gen):stepsPerDay},gen.volumeNoiseFactor=function(x){return arguments.length?(volumeNoiseFactor=x,gen):volumeNoiseFactor},gen.filter=function(x){return arguments.length?(filter=x,gen):filter},gen}
//  https://www.quandl.com/docs/api#datasets
function quandl(){function defaultColumnNameMap(colName){return colName[0].toLowerCase()+colName.substr(1)}var database="YAHOO",dataset="GOOG",apiKey=null,start=null,end=null,rows=null,descending=!1,collapse=null,columnNameMap=defaultColumnNameMap,quandl=function(cb){var params=[];null!=apiKey&&params.push("api_key="+apiKey),null!=start&&params.push("start_date="+start.toISOString().substring(0,10)),null!=end&&params.push("end_date="+end.toISOString().substring(0,10)),null!=rows&&params.push("rows="+rows),descending||params.push("order=asc"),null!=collapse&&params.push("collapse="+collapse);var url="https://www.quandl.com/api/v3/datasets/"+database+"/"+dataset+"/data.json?"+params.join("&");d3.json(url,function(error,data){if(error)return void cb(error);var datasetData=data.dataset_data,nameMapping=columnNameMap||function(n){return n},colNames=datasetData.column_names.map(function(n,i){return[i,nameMapping(n)]}).filter(function(v){return v[1]}),mappedData=datasetData.data.map(function(d){var output={};return colNames.forEach(function(v){output[v[1]]=0===v[0]?new Date(d[v[0]]):d[v[0]]}),output});cb(error,mappedData)})};
...
return quandl.database=function(x){return arguments.length?(database=x,quandl):database},quandl.dataset=function(x){return arguments.length?(dataset=x,quandl):dataset},quandl.apiKey=function(x){return arguments.length?(apiKey=x,quandl):apiKey},quandl.start=function(x){return arguments.length?(start=x,quandl):start},quandl.end=function(x){return arguments.length?(end=x,quandl):end},quandl.rows=function(x){return arguments.length?(rows=x,quandl):rows},quandl.descending=function(x){return arguments.length?(descending=x,quandl):descending},quandl.collapse=function(x){return arguments.length?(collapse=x,quandl):collapse},quandl.columnNameMap=function(x){return arguments.length?(columnNameMap=x,quandl):columnNameMap},quandl.defaultColumnNameMap=defaultColumnNameMap,quandl}
// https://docs.exchange.coinbase.com/#market-data
function coinbase(){var product="BTC-USD",start=null,end=null,granularity=null,coinbase=function(cb){var params=[];null!=start&&params.push("start="+start.toISOString()),null!=end&&params.push("end="+end.toISOString()),null!=granularity&&params.push("granularity="+granularity);var url="https://api.exchange.coinbase.com/products/"+product+"/candles?"+params.join("&");d3.json(url,function(error,data){return error?void cb(error):(data=data.map(function(d){return{date:new Date(1e3*d[0]),open:d[3],high:d[2],low:d[1],close:d[4],volume:d[5]}}),void cb(error,data))})};return coinbase.product=function(x){return arguments.length?(product=x,coinbase):product},coinbase.start=function(x){return arguments.length?(start=x,coinbase):start},coinbase.end=function(x){return arguments.length?(end=x,coinbase):end},coinbase.granularity=function(x){return arguments.length?(granularity=x,coinbase):granularity},coinbase}function smallMultiples(xScale,yScale){function classedDataJoin(clazz){return dataJoin().selector("g."+clazz).element("g").attr("class",clazz)}xScale=xScale||d3.scale.linear(),yScale=yScale||d3.scale.linear();var padding=10,columns=9,decorate=noop,plotArea=_line(),margin={bottom:30,right:30},values=function(d){return d.values},key=function(d){return d.key},xAxis=axis().ticks(2),yAxis=axis().orient("right").ticks(3),dataJoin$$=classedDataJoin("multiple"),xAxisDataJoin=classedDataJoin("x-axis"),yAxisDataJoin=classedDataJoin("y-axis"),multiples=function(selection){selection.each(function(data,index){function translationForMultiple(row,column){return{xOffset:(multipleWidth+padding)*row,yOffset:(multipleHeight+padding)*column}}var container=d3.select(this),expandedMargin=expandMargin(margin);expandedMargin.position="absolute";var svg=container.selectAll("svg").data([data]);svg.enter().append("svg").layout("flex",1).append("g").attr("class","multiples-chart");var plotAreaContainer=svg.select("g").layout(expandedMargin);container.layout();var rows=Math.ceil(data.length/columns),multipleWidth=plotAreaContainer.layout("width")/columns-padding,multipleHeight=plotAreaContainer.layout("height")/rows-padding;setRange(xScale,[0,multipleWidth]),setRange(yScale,[multipleHeight,0]),plotArea.xScale(xScale).yScale(yScale);
// create a container for each multiple chart
var multipleContainer=dataJoin$$(plotAreaContainer,data);multipleContainer.attr("transform",function(d,i){var translation=translationForMultiple(i%columns,Math.floor(i/columns));return"translate("+translation.xOffset+","+translation.yOffset+")"});
// within each, add an inner 'g' and background rect
var inner=multipleContainer.enter().append("g");inner.append("rect").attr("class","background"),inner.append("g").attr("transform","translate("+multipleWidth/2+", 0)").append("text").attr("class","label").text(key),
// on update, call the plotArea and size the rect element
multipleContainer.select("g").datum(values).call(plotArea),multipleContainer.select("rect").attr({width:multipleWidth,height:multipleHeight}),decorate(multipleContainer,data,index);var xAxisContainer=xAxisDataJoin(plotAreaContainer,d3.range(columns));xAxisContainer.attr("transform",function(d,i){var row="bottom"===xAxis.orient()?rows:0,offset="bottom"===xAxis.orient()?0:-padding,translation=translationForMultiple(i,row);return"translate("+translation.xOffset+","+(translation.yOffset+offset)+")"}),xAxis.scale(xScale),xAxisContainer.call(xAxis);var yAxisContainer=yAxisDataJoin(plotAreaContainer,d3.range(rows));yAxisContainer.attr("transform",function(d,i){var column="left"===yAxis.orient()?0:columns,offset="left"===yAxis.orient()?-padding:0,translation=translationForMultiple(column,i);return"translate("+(translation.xOffset+offset)+","+translation.yOffset+")"}),yAxis.scale(yScale),yAxisContainer.call(yAxis)})},scaleExclusions=[/range\w*/,// the scale range is set via the component layout
/tickFormat/];return rebindAll(multiples,xScale,"x",scaleExclusions),rebindAll(multiples,yScale,"y",scaleExclusions),rebindAll(multiples,xAxis,"x"),rebindAll(multiples,yAxis,"y"),multiples.columns=function(x){return arguments.length?(columns=x,multiples):columns},multiples.margin=function(x){return arguments.length?(margin=x,multiples):margin},multiples.padding=function(x){return arguments.length?(padding=x,multiples):padding},multiples.plotArea=function(x){return arguments.length?(plotArea=x,multiples):plotArea},multiples.values=function(x){return arguments.length?(values=x,multiples):values},multiples.key=function(x){return arguments.length?(key=x,multiples):key},multiples.decorate=function(x){return arguments.length?(decorate=x,multiples):decorate},multiples}function sparkline(){
// creates an array with four elements, representing the high, low, open and close
// values of the given array
function highLowOpenClose(data){function elementWithYValue(value){return data.filter(function(d){return yValueAccessor(d)===value})[0]}var xValueAccessor=sparkline.xValue(),yValueAccessor=sparkline.yValue(),high=d3.max(data,yValueAccessor),low=d3.min(data,yValueAccessor);return[{x:xValueAccessor(data[0]),y:yValueAccessor(data[0])},{x:xValueAccessor(elementWithYValue(high)),y:high},{x:xValueAccessor(elementWithYValue(low)),y:low},{x:xValueAccessor(data[data.length-1]),y:yValueAccessor(data[data.length-1])}]}var xScale=exportedScale(),yScale=d3.scale.linear(),radius=2,line=_line(),point$$=point().xValue(function(d){return d.x}).yValue(function(d){return d.y}).decorate(function(sel){sel.attr("class",function(d,i){switch(i){case 0:return"open";case 1:return"high";case 2:return"low";case 3:return"close"}})}),multi=_multi().series([line,point$$]).mapping(function(series){switch(series){case point$$:return highLowOpenClose(this);default:return this}}),sparkline=function(selection){point$$.size(radius*radius*Math.PI),selection.each(function(data){var container=d3.select(this),dimensions=innerDimensions(this),margin=radius;xScale.range([margin,dimensions.width-margin]),yScale.range([dimensions.height-margin,margin]),multi.xScale(xScale).yScale(yScale),container.call(multi)})};return rebind(sparkline,xScale,{xDiscontinuityProvider:"discontinuityProvider",xDomain:"domain"}),rebind(sparkline,yScale,{yDomain:"domain"}),rebind(sparkline,line,"xValue","yValue"),sparkline.xScale=function(){return xScale},sparkline.yScale=function(){return yScale},sparkline.radius=function(x){return arguments.length?(radius=x,sparkline):radius},sparkline}function linearTimeSeries(){var xAxisHeight=20,yAxisWidth=0,plotArea=_line(),xScale=exportedScale(),yScale=d3.scale.linear(),xAxis=axis().scale(xScale).orient("bottom"),yAxis=axis().scale(yScale).orient("left"),linearTimeSeries=function(selection){selection.each(function(data){var container=d3.select(this),plotAreaLayout={position:"absolute",top:0,right:yAxisWidth,bottom:xAxisHeight,left:0},background=container.selectAll("rect.background").data([data]);background.enter().append("rect").attr("class","background").layout(plotAreaLayout);var plotAreaContainer=container.selectAll("svg.plot-area").data([data]);plotAreaContainer.enter().append("svg").attr({"class":"plot-area"}).layout(plotAreaLayout);var xAxisContainer=container.selectAll("g.x-axis").data([data]);xAxisContainer.enter().append("g").attr("class","axis x-axis").layout({position:"absolute",left:0,bottom:0,right:yAxisWidth,height:xAxisHeight});var yAxisContainer=container.selectAll("g.y-axis").data([data]);yAxisContainer.enter().append("g").attr("class","axis y-axis").layout({position:"absolute",top:0,right:0,bottom:xAxisHeight,width:yAxisWidth}),container.layout(),xScale.range([0,xAxisContainer.layout("width")]),yScale.range([yAxisContainer.layout("height"),0]),xAxisContainer.call(xAxis),yAxisContainer.call(yAxis),plotArea.xScale(xScale).yScale(yScale),plotAreaContainer.call(plotArea)})};
// Exclude scale when rebinding the axis properties because this component
// is responsible for providing the required scale.
return rebind(linearTimeSeries,xScale,{xDiscontinuityProvider:"discontinuityProvider",xDomain:"domain",xNice:"nice"}),rebind(linearTimeSeries,yScale,{yDomain:"domain",yNice:"nice"}),rebindAll(linearTimeSeries,xAxis,"x","scale"),rebindAll(linearTimeSeries,yAxis,"y","scale"),linearTimeSeries.xScale=function(){return xScale},linearTimeSeries.yScale=function(){return yScale},linearTimeSeries.plotArea=function(x){return arguments.length?(plotArea=x,linearTimeSeries):plotArea},linearTimeSeries.xAxisHeight=function(x){return arguments.length?(xAxisHeight=x,linearTimeSeries):xAxisHeight},linearTimeSeries.yAxisWidth=function(x){return arguments.length?(yAxisWidth=x,linearTimeSeries):yAxisWidth},linearTimeSeries}function legend(){var tableDecorate=noop,rowDecorate=noop,items=[["datum",function(d){return d.datum}]],tableDataJoin=dataJoin().selector("table.legend").element("table").attr("class","legend"),rowDataJoin=dataJoin().selector("tr.row").element("tr").attr("class","row"),legend=function(selection){selection.each(function(data,index){var container=d3.select(this),legendData=items.map(function(item,i){return{datum:data,header:d3.functor(item[0]),value:d3.functor(item[1])}}),table=tableDataJoin(container,[legendData]),trUpdate=rowDataJoin(table),trEnter=trUpdate.enter();trEnter.append("th"),trEnter.append("td"),trUpdate.select("th").html(function(d,i){return d.header.call(this,d.datum,i)}),trUpdate.select("td").html(function(d,i){return d.value.call(this,d.datum,i)}),tableDecorate(table,data,index),rowDecorate(trUpdate,data,index)})};return legend.items=function(x){return arguments.length?(items=x,legend):items},legend.rowDecorate=function(x){return arguments.length?(rowDecorate=x,legend):rowDecorate},legend.tableDecorate=function(x){return arguments.length?(tableDecorate=x,legend):tableDecorate},legend}function cartesian(xScale,yScale){xScale=xScale||d3.scale.linear(),yScale=yScale||d3.scale.linear();var margin={bottom:30,right:30},yLabel="",xLabel="",xBaseline=null,yBaseline=null,chartLabel="",plotArea=_line(),decorate=noop,xAxis=_axis().orient("bottom").baseline(function(){if(null!==xBaseline)return yScale(xBaseline.apply(this,arguments));var r=range(yScale);return"bottom"===xAxis.orient()?r[0]:r[1]}),yAxis=_axis().orient("right").baseline(function(){if(null!==yBaseline)return xScale(yBaseline.apply(this,arguments));var r=range(xScale);return"left"===yAxis.orient()?r[0]:r[1]}),containerDataJoin=dataJoin().selector("svg.cartesian-chart").element("svg").attr({"class":"cartesian-chart","layout-style":"flex: 1"}),cartesian=function(selection){selection.each(function(data,index){var container=d3.select(this),svg=containerDataJoin(container,[data]);svg.enter().html('<g class="title">                         <g layout-style="height: 0; width: 0">                             <text class="label"/>                         </g>                     </g>                     <g class="y-axis">                         <g layout-style="height: 0; width: 0">                             <text class="label"/>                         </g>                     </g>                     <g class="x-axis">                         <g layout-style="height: 0; width: 0">                             <text class="label"/>                         </g>                     </g>                     <g class="plot-area-container">                         <rect class="background"                             layout-style="position: absolute; top: 0; bottom: 0; left: 0; right: 0"/>                         <g class="axes-container"                             layout-style="position: absolute; top: 0; bottom: 0; left: 0; right: 0">                             <g class="x-axis" layout-style="height: 0; width: 0"/>                             <g class="y-axis" layout-style="height: 0; width: 0"/>                         </g>                         <svg class="plot-area"                             layout-style="position: absolute; top: 0; bottom: 0; left: 0; right: 0"/>                     </g>');var expandedMargin=expandMargin(margin);svg.select(".plot-area-container").layout({position:"absolute",top:expandedMargin.top,left:expandedMargin.left,bottom:expandedMargin.bottom,right:expandedMargin.right}),svg.select(".title").layout({position:"absolute",top:0,alignItems:"center",left:expandedMargin.left,right:expandedMargin.right});var yAxisLayout={position:"absolute",top:expandedMargin.top,bottom:expandedMargin.bottom,alignItems:"center",flexDirection:"row"};yAxisLayout[yAxis.orient()]=0,svg.select(".y-axis").attr("class","y-axis "+yAxis.orient()).layout(yAxisLayout);var xAxisLayout={position:"absolute",left:expandedMargin.left,right:expandedMargin.right,alignItems:"center"};xAxisLayout[xAxis.orient()]=0,svg.select(".x-axis").attr("class","x-axis "+xAxis.orient()).layout(xAxisLayout),
// perform the flexbox / css layout
container.layout(),
// update the label text
svg.select(".title .label").text(chartLabel),svg.select(".y-axis .label").text(yLabel).attr("transform","right"===yAxis.orient()?"rotate(90)":"rotate(-90)"),svg.select(".x-axis .label").text(xLabel);
// set the axis ranges
var plotAreaContainer=svg.select(".plot-area");setRange(xScale,[0,plotAreaContainer.layout("width")]),setRange(yScale,[plotAreaContainer.layout("height"),0]),
// render the axes
xAxis.xScale(xScale).yScale(d3.scale.identity()),yAxis.yScale(yScale).xScale(d3.scale.identity()),svg.select(".axes-container .x-axis").call(xAxis),svg.select(".axes-container .y-axis").call(yAxis),
// render the plot area
plotArea.xScale(xScale).yScale(yScale),plotAreaContainer.call(plotArea),decorate(svg,data,index)})},scaleExclusions=[/range\w*/,// the scale range is set via the component layout
/tickFormat/];rebindAll(cartesian,xScale,"x",scaleExclusions),rebindAll(cartesian,yScale,"y",scaleExclusions);var axisExclusions=["baseline",// the axis baseline is adapted so is not exposed directly
"xScale","yScale"];return rebindAll(cartesian,xAxis,"x",axisExclusions),rebindAll(cartesian,yAxis,"y",axisExclusions),cartesian.xBaseline=function(x){return arguments.length?(xBaseline=d3.functor(x),cartesian):xBaseline},cartesian.yBaseline=function(x){return arguments.length?(yBaseline=d3.functor(x),cartesian):yBaseline},cartesian.chartLabel=function(x){return arguments.length?(chartLabel=x,cartesian):chartLabel},cartesian.plotArea=function(x){return arguments.length?(plotArea=x,cartesian):plotArea},cartesian.xLabel=function(x){return arguments.length?(xLabel=x,cartesian):xLabel},cartesian.margin=function(x){return arguments.length?(margin=x,cartesian):margin},cartesian.yLabel=function(x){return arguments.length?(yLabel=x,cartesian):yLabel},cartesian.decorate=function(x){return arguments.length?(decorate=x,cartesian):decorate},cartesian}function gridline(){var xScale=d3.time.scale(),yScale=d3.scale.linear(),xTicks=10,yTicks=10,xDecorate=noop,yDecorate=noop,xLineDataJoin=dataJoin().selector("line.x").element("line").attr("class","x gridline"),yLineDataJoin=dataJoin().selector("line.y").element("line").attr("class","y gridline"),gridlines=function(selection){selection.each(function(data,index){var xData=xScale.ticks(xTicks),xLines=xLineDataJoin(this,xData);xLines.attr({x1:xScale,x2:xScale,y1:yScale.range()[0],y2:yScale.range()[1]}),xDecorate(xLines,xData,index);var yData=yScale.ticks(yTicks),yLines=yLineDataJoin(this,yData);yLines.attr({x1:xScale.range()[0],x2:xScale.range()[1],y1:yScale,y2:yScale}),yDecorate(yLines,yData,index)})};return gridlines.xScale=function(x){return arguments.length?(xScale=x,gridlines):xScale},gridlines.yScale=function(x){return arguments.length?(yScale=x,gridlines):yScale},gridlines.xTicks=function(x){return arguments.length?(xTicks=x,gridlines):xTicks},gridlines.yTicks=function(x){return arguments.length?(yTicks=x,gridlines):yTicks},gridlines.yDecorate=function(x){return arguments.length?(yDecorate=x,gridlines):yDecorate},gridlines.xDecorate=function(x){return arguments.length?(xDecorate=x,gridlines):xDecorate},rebind(gridlines,xLineDataJoin,{xKey:"key"}),rebind(gridlines,yLineDataJoin,{yKey:"key"}),gridlines}function band(){var x0,x1,y0,y1,xScale=d3.time.scale(),yScale=d3.scale.linear(),x0Scaled=function(){return range(xScale)[0]},x1Scaled=function(){return range(xScale)[1]},y0Scaled=function(){return range(yScale)[0]},y1Scaled=function(){return range(yScale)[1]},decorate=noop,dataJoin$$=dataJoin().selector("g.annotation").element("g").attr("class","annotation"),band=function(selection){selection.each(function(data,index){var container=d3.select(this),g=dataJoin$$(container,data);g.enter().append("path").classed("band",!0);var pathGenerator=bar().horizontalAlign("right").verticalAlign("top").x(x0Scaled).y(y0Scaled).height(function(){return y1Scaled.apply(this,arguments)-y0Scaled.apply(this,arguments)}).width(function(){return x1Scaled.apply(this,arguments)-x0Scaled.apply(this,arguments)});g.select("path").attr("d",function(d,i){
// the path generator is being used to render a single path, hence
// an explicit index is provided
return pathGenerator.call(this,[d],i)}),decorate(g,data,index)})};return band.xScale=function(x){return arguments.length?(xScale=x,band):xScale},band.yScale=function(x){return arguments.length?(yScale=x,band):yScale},band.decorate=function(x){return arguments.length?(decorate=x,band):decorate},band.x0=function(x){return arguments.length?(x0=d3.functor(x),x0Scaled=function(){return xScale(x0.apply(this,arguments))},band):x0},band.x1=function(x){return arguments.length?(x1=d3.functor(x),x1Scaled=function(){return xScale(x1.apply(this,arguments))},band):x1},band.y0=function(x){return arguments.length?(y0=d3.functor(x),y0Scaled=function(){return yScale(y0.apply(this,arguments))},band):y0},band.y1=function(x){return arguments.length?(y1=d3.functor(x),y1Scaled=function(){return yScale(y1.apply(this,arguments))},band):y1},band}computeLayout="default"in computeLayout?computeLayout["default"]:computeLayout,d3="default"in d3?d3["default"]:d3;var _scale={get isOrdinal(){return isOrdinal},get range(){return range},get setRange(){return setRange}},_fn={get context(){return context},get identity(){return identity},get index(){return _index},get noop(){return noop}};d3.selection.prototype.layoutSuspended=layoutSuspended,d3.transition.prototype.layoutSuspended=layoutSuspended,d3.selection.prototype.layout=layoutSelection,d3.transition.prototype.layout=layoutSelection;
...
var effectivelyZero=1e-6,util={dataJoin:dataJoin,expandMargin:expandMargin,extent:extent,fn:_fn,fractionalBarWidth:fractionalBarWidth,innerDimensions:innerDimensions,rebind:rebind,rebindAll:rebindAll,scale:_scale,noSnap:noSnap,pointSnap:pointSnap,seriesPointSnap:seriesPointSnap,seriesPointSnapXOnly:seriesPointSnapXOnly,seriesPointSnapYOnly:seriesPointSnapYOnly,render:render},tool={crosshair:crosshair,fibonacciFan:fibonacciFan,measure:measure},svg={axis:axis,bar:bar,candlestick:candlestick,ohlc:ohlc,errorBar:errorBar},stacked={area:area,bar:_bar,stack:_stack,line:__line},_series={area:_area,axis:_axis,bar:barSeries,candlestick:_candlestick,cycle:cycle,line:_line,multi:_multi,ohlc:_ohlc,point:point,stacked:stacked,groupedBar:groupedBar,xyBase:xyBase,ohlcBase:ohlcBase,errorBar:_errorBar};exportedScale.tickTransformer=tickTransformer;var __scale={discontinuity:{identity:_identity,skipWeekends:skipWeekends},dateTime:exportedScale},renderer={bollingerBands:bollingerBands,macd:macd,relativeStrengthIndex:relativeStrengthIndex,stochasticOscillator:stochasticOscillator,forceIndex:forceIndex,envelope:envelope,elderRay:elderRay},______calculator={bollingerBands:_bollingerBands,exponentialMovingAverage:exponentialMovingAverage,macd:_____calculator,percentageChange:percentageChange,relativeStrengthIndex:____calculator,stochasticOscillator:___calculator,slidingWindow:calculator,undefinedInputAdapter:undefinedInputAdapter,forceIndex:__calculator,envelope:envelopeCalculator,elderRay:_calculator},algorithm={bollingerBands:__bollingerBands,calculator:______calculator,exponentialMovingAverage:_exponentialMovingAverage,macd:_macd,merge:merge,movingAverage:movingAverage,relativeStrengthIndex:_relativeStrengthIndex,stochasticOscillator:_stochasticOscillator,forceIndex:_forceIndex,envelope:_envelope,elderRay:_elderRay},_indicator={algorithm:algorithm,renderer:renderer},sampler={modeMedian:modeMedian},random={financial:financial,walk:walk},feed={coinbase:coinbase,quandl:quandl},_data={feed:feed,random:random,spread:spread,sampler:sampler},chart={cartesian:cartesian,legend:legend,linearTimeSeries:linearTimeSeries,sparkline:sparkline,smallMultiples:smallMultiples},annotation={band:band,gridline:gridline,line:line},version="4.3.1",fc={annotation:annotation,chart:chart,data:_data,indicator:_indicator,scale:__scale,series:_series,svg:svg,tool:tool,util:util,version:version};return fc}),/*! jQuery v2.1.4 | (c) 2005, 2015 jQuery Foundation, Inc. | jquery.org/license */
...
var global=null;global=(1,eval)("this"),global.sc={behavior:{},chart:{},data:{feed:{coinbase:{}}},menu:{data:{},generator:{},primary:{},secondary:{}},model:{menu:{}},series:{},util:{domain:{}}}}(),function(d3,fc,sc){"use strict";sc.event={crosshairChange:"crosshairChange",viewChange:"viewChange",messageReceived:"messageReceived",dataLoaded:"dataLoaded",dataProductChange:"dataProductChange",dataPeriodChange:"dataPeriodChange",resetToLatest:"resetToLatest",toggleSlideout:"toggleSlideout",primaryChartSeriesChange:"primaryChartSeriesChange",primaryChartYValueAccessorChange:"primaryChartYValueAccessorChange",primaryChartIndicatorChange:"primaryChartIndicatorChange",secondaryChartChange:"secondaryChartChange"}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.headMenu=function(initialProducts,initialSelectedProduct,initialSelectedPeriod){return{products:initialProducts,selectedProduct:initialSelectedProduct,selectedPeriod:initialSelectedPeriod}}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.legend=function(initialProduct,initialPeriod){return{data:void 0,product:initialProduct,period:initialPeriod}}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.menu.indicatorOptions=function(){var movingAverage=fc.series.line().decorate(function(select){select.enter().classed("movingAverage",!0)}).yValue(function(d){return d.movingAverage});return[sc.menu.option("Moving Average","movingAverage",movingAverage),sc.menu.option("Bollinger Bands","bollinger",fc.indicator.renderer.bollingerBands())]}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.menu.secondaryChartOptions=function(){return[sc.menu.option("RSI","secondary-rsi",sc.chart.rsi()),sc.menu.option("MACD","secondary-macd",sc.chart.macd()),sc.menu.option("Volume","secondary-volume",sc.chart.volume())]}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.menu.seriesOptions=function(){var candlestick=sc.series.candlestick();candlestick.isPadded=!0;var candlestickOption=sc.menu.option("Candlestick","candlestick",candlestick);candlestickOption.isSelected=!0;var ohlc=fc.series.ohlc();ohlc.isPadded=!0;
// TODO: Could 'isLine' go on primary chart series model instead?
var line=fc.series.line();line.isLine=!0;var point=fc.series.point();return point.isPadded=!0,[candlestickOption,sc.menu.option("OHLC","ohlc",ohlc),sc.menu.option("Line","line",line),sc.menu.option("Point","point",point),sc.menu.option("Area","area",fc.series.area())]}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.menu.side=function(){
// TODO: Instantiate series/indicator components outside of menu model?
return{seriesOptions:sc.model.menu.seriesOptions(),yValueAccessorOptions:sc.model.menu.yValueAccessorOptions(),// TODO: remove? #263.
indicatorOptions:sc.model.menu.indicatorOptions(),secondaryChartOptions:sc.model.menu.secondaryChartOptions()}}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.menu.yValueAccessorOptions=function(){var closeOption=sc.menu.option("Close","close",function(d){return d.close});return closeOption.isSelected=!0,[sc.menu.option("Open","open",function(d){return d.open}),sc.menu.option("High","high",function(d){return d.high}),sc.menu.option("Low","low",function(d){return d.low}),closeOption]}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.nav=function(){return{data:[],viewDomain:[],padding:43200,trackingLatest:!0}}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.period=function(config){return config=config||{},{display:config.display||"1 day",seconds:config.seconds||86400,d3TimeInterval:config.d3TimeInterval||{unit:d3.time.day,value:1},timeFormat:d3.time.format(config.timeFormat||"%b %d")}}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.primaryChart=function(initialProduct){var model={data:[],trackingLatest:!0,viewDomain:[],padding:43200,selectorsChanged:!0},_product=initialProduct;Object.defineProperty(model,"product",{get:function(){return _product},set:function(newValue){_product=newValue,model.selectorsChanged=!0}});var candlestick=sc.series.candlestick();candlestick.isPadded=!0;var _series=sc.menu.option("Candlestick","candlestick",candlestick);Object.defineProperty(model,"series",{get:function(){return _series},set:function(newValue){_series=newValue,model.selectorsChanged=!0}});var _yValueAccessor={option:function(d){return d.close}};Object.defineProperty(model,"yValueAccessor",{get:function(){return _yValueAccessor},set:function(newValue){_yValueAccessor=newValue,model.selectorsChanged=!0}});var _indicators=[];return Object.defineProperty(model,"indicators",{get:function(){return _indicators},set:function(newValue){_indicators=newValue,model.selectorsChanged=!0}}),model}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.product=function(config){return{display:config.display||"Unspecified Product",priceFormat:d3.format(config.priceFormat||".2f"),volumeFormat:d3.format(config.volumeFormat||"s"),periods:config.periods||[]}}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.secondaryChart=function(initialProduct){return{data:[],viewDomain:[],padding:43200,trackingLatest:!0,product:initialProduct}}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.model.xAxis=function(initialPeriod){return{viewDomain:[],padding:43200,period:initialPeriod}}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.chart.legend=function(){function legend(selection){selection.each(function(model){var container=d3.select(this);if(formatPrice=model.product.priceFormat,formatVolume=model.product.volumeFormat,formatTime=model.period.timeFormat,null==model.data||model.data!==lastDataPointDisplayed){lastDataPointDisplayed=model.data;var span=container.selectAll("span").data(legendItems);span.enter().append("span").attr("class",function(d,i){return i%2===0?"legendLabel":"legendValue"}),span.text(function(d,i){return i%2===0?d:model.data?d(model.data):""})}})}var formatPrice,formatVolume,formatTime,lastDataPointDisplayed,legendItems=["T",function(d){return formatTime(d.date)},"O",function(d){return formatPrice(d.open)},"H",function(d){return formatPrice(d.high)},"L",function(d){return formatPrice(d.low)},"C",function(d){return formatPrice(d.close)},"V",function(d){return formatVolume(d.volume)}];return legend}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.chart.macd=function(){function macd(selection){var model=selection.datum();algorithm(model.data);var maxYExtent=d3.max(model.data,function(d){return Math.abs(d.macd.macd)}),paddedYExtent=sc.util.domain.padYDomain([-maxYExtent,maxYExtent],.04);chart.trackingLatest(model.trackingLatest).padding(model.padding).xDomain(model.viewDomain).yDomain(paddedYExtent),selection.datum(model.data).call(chart)}var dispatch=d3.dispatch(sc.event.viewChange),zeroLine=fc.annotation.line().value(0).label(""),renderer=fc.indicator.renderer.macd(),algorithm=fc.indicator.algorithm.macd(),chart=sc.chart.secondary().series([zeroLine,renderer]).yTicks(5).mapping(function(series){return series===zeroLine?[0]:this}).decorate(function(g){g.enter().attr("class",function(d,i){return["multi zero","multi"][i]})}).on(sc.event.viewChange,function(domain){dispatch[sc.event.viewChange](domain)});return d3.rebind(macd,dispatch,"on"),macd.dimensionChanged=function(container){chart.dimensionChanged(container)},macd}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.chart.nav=function(){function nav(selection){var navbarContainer=selection.select("#navbar-container"),navbarReset=selection.select("#navbar-reset"),model=navbarContainer.datum();viewScale.domain(model.viewDomain);var resetButton=navbarReset.selectAll("g").data([model]);resetButton.enter().append("g").attr("class","reset-button").on("click",function(){dispatch[sc.event.resetToLatest]()}).append("path").attr("d","M1.5 1.5h13.438L23 20.218 14.937 38H1.5l9.406-17.782L1.5 1.5z"),resetButton.classed("active",!model.trackingLatest);var filteredData=sc.util.domain.filterDataInDateRange(fc.util.extent().fields("date")(model.data),model.data),yExtent=fc.util.extent().fields(["low","high"])(filteredData),xExtent=fc.util.extent().fields("date")(model.data),paddedXExtent=model.padding?sc.util.domain.padTimeExtent(xExtent,model.data,model.padding):xExtent;navChart.xDomain(paddedXExtent).yDomain(yExtent),brush.on("brush",function(){if(brush.extent()[0][0]-brush.extent()[1][0]!==0){var domain=[brush.extent()[0][0],brush.extent()[1][0]],unpaddedDomain=model.padding?sc.util.domain.padTimeExtent(domain,model.data,-model.padding):domain;dispatch[sc.event.viewChange](unpaddedDomain)}}).on("brushend",function(){if(brush.extent()[0][0]-brush.extent()[1][0]===0){var domain=sc.util.domain.centerOnDate(viewScale.domain(),model.data,brush.extent()[0][0]),unpaddedDomain=model.padding?sc.util.domain.padTimeExtent(domain,model.data,-model.padding):domain;dispatch[sc.event.viewChange](unpaddedDomain)}}),navChart.plotArea(navMulti),navbarContainer.call(navChart);
// Allow to zoom using mouse, but disable panning
var zoom=sc.behavior.zoom(layoutWidth).scale(viewScale).trackingLatest(model.trackingLatest).allowPan(!1).on("zoom",function(domain){dispatch[sc.event.viewChange](domain)});navbarContainer.select(".plot-area").call(zoom)}var layoutWidth,dispatch=d3.dispatch(sc.event.viewChange,sc.event.resetToLatest),navChart=fc.chart.cartesian(fc.scale.dateTime(),d3.scale.linear()).yTicks(0).margin({bottom:40}),viewScale=fc.scale.dateTime(),area=fc.series.area().yValue(function(d){return d.close}),line=fc.series.line().yValue(function(d){return d.close}),brush=d3.svg.brush(),navMulti=fc.series.multi().series([area,line,brush]).mapping(function(series){return series===brush&&brush.extent([[viewScale.domain()[0],navChart.yDomain()[0]],[viewScale.domain()[1],navChart.yDomain()[1]]]),this.data});return d3.rebind(nav,dispatch,"on"),nav.dimensionChanged=function(container){layoutWidth=parseInt(container.style("width")),viewScale.range([0,layoutWidth])},nav}}(d3,fc,sc),function(d3,fc,sc){"use strict";function calculateCloseAxisTagPath(width,height){var h2=height/2;return[[0,0],[h2,-h2],[width,-h2],[width,h2],[h2,h2],[0,0]]}function produceAnnotatedTickValues(scale,annotation){for(var annotatedTickValues=scale.ticks.apply(scale,[]),extent=scale.domain(),i=0;i<annotation.length;i++)annotation[i]>extent[0]&&annotation[i]<extent[1]&&annotatedTickValues.push(annotation[i]);return annotatedTickValues}function findTotalYExtent(visibleData,currentSeries,currentIndicators){var extentAccessor;switch(currentSeries.valueString){case"candlestick":case"ohlc":extentAccessor=[currentSeries.option.yLowValue(),currentSeries.option.yHighValue()];break;case"line":case"point":extentAccessor=currentSeries.option.yValue();break;case"area":extentAccessor=currentSeries.option.y1Value();break;default:throw new Error("Main series given to chart does not have expected interface")}var extent=fc.util.extent().fields(extentAccessor)(visibleData);if(currentIndicators.length){var indicators=currentIndicators.map(function(indicator){return indicator.valueString}),movingAverageShown=-1!==indicators.indexOf("movingAverage"),bollingerBandsShown=-1!==indicators.indexOf("bollinger");if(bollingerBandsShown){var bollingerBandsVisibleDataObject=visibleData.map(function(d){return d.bollingerBands}),bollingerBandsExtent=fc.util.extent().fields(["lower","upper"])(bollingerBandsVisibleDataObject);extent[0]=d3.min([bollingerBandsExtent[0],extent[0]]),extent[1]=d3.max([bollingerBandsExtent[1],extent[1]])}if(movingAverageShown){var movingAverageExtent=fc.util.extent().fields("movingAverage")(visibleData);extent[0]=d3.min([movingAverageExtent[0],extent[0]]),extent[1]=d3.max([movingAverageExtent[1],extent[1]])}if(!movingAverageShown&&!bollingerBandsShown)throw new Error("Unexpected indicator type")}return extent}sc.chart.primary=function(){function updateMultiSeries(series){var baseChart=[gridlines,currentSeries.option,closeLine],indicators=currentIndicators.map(function(indicator){return indicator.option});series(baseChart.concat(indicators)),
// add crosshair last to have it on top
series(series().concat(crosshair))}function updateYValueAccessorUsed(){switch(movingAverage.value(currentYValueAccessor),bollingerAlgorithm.value(currentYValueAccessor),closeLine.value(currentYValueAccessor),currentSeries.valueString){case"line":case"point":case"area":currentSeries.option.yValue(currentYValueAccessor)}}function bandCrosshair(data){var width=currentSeries.option.width(data);crosshair.decorate(function(selection){selection.enter().classed("band",!0),selection.enter().selectAll("line").classed("hidden",function(){return this.hasAttribute("x2")?!0:this.hasAttribute("y2")?!1:void 0}),selection.selectAll("line").style("stroke-width",function(){return this.hasAttribute("y2")?width:void 0})})}function lineCrosshair(selection){selection.enter().classed("band",!1).selectAll("line").style("stroke-width",null)}function updateCrosshairDecorate(data){"candlestick"===currentSeries.valueString||"ohlc"===currentSeries.valueString?bandCrosshair(data):crosshair.decorate(lineCrosshair)}function primary(selection){var model=selection.datum();model.selectorsChanged&&selectorsChanged(model),primaryChart.xDomain(model.viewDomain),crosshair.snap(fc.util.seriesPointSnapXOnly(currentSeries.option,model.data)),updateCrosshairDecorate(model.data),movingAverage(model.data),bollingerAlgorithm(model.data);
// Scale y axis
var visibleData=sc.util.domain.filterDataInDateRange(primaryChart.xDomain(),model.data),yExtent=findTotalYExtent(visibleData,currentSeries,currentIndicators),paddedYExtent=sc.util.domain.padYDomain(yExtent,.04);primaryChart.yDomain(paddedYExtent);
// Find current tick values and add close price to this list, then set it explicitly below
var latestPrice=currentYValueAccessor(model.data[model.data.length-1]),tickValues=produceAnnotatedTickValues(yScale,[latestPrice]);primaryChart.yTickValues(tickValues).yDecorate(function(s){s.selectAll(".tick").filter(function(d){return d===latestPrice}).classed("closeLine",!0).select("path").attr("d",function(d){return d3.svg.area()(calculateCloseAxisTagPath(yAxisWidth,14))})}),
// Redraw
primaryChart.plotArea(multi),selection.call(primaryChart);var zoom=sc.behavior.zoom(zoomWidth).scale(xScale).trackingLatest(model.trackingLatest).padding(model.padding).on("zoom",function(domain){dispatch[sc.event.viewChange](domain)});selection.select(".plot-area").call(zoom)}var zoomWidth,yAxisWidth=60,dispatch=d3.dispatch(sc.event.viewChange,sc.event.crosshairChange),currentSeries=sc.menu.option("Candlestick","candlestick",sc.series.candlestick()),currentYValueAccessor=function(d){return d.close},currentIndicators=[],crosshairData=[],crosshair=fc.tool.crosshair().xLabel("").yLabel("").on("trackingmove",function(crosshairData){dispatch.crosshairChange(crosshairData[0].datum)}).on("trackingend",function(){dispatch.crosshairChange(void 0)}),gridlines=fc.annotation.gridline().yTicks(5).xTicks(0),closeLine=fc.annotation.line().orient("horizontal").value(currentYValueAccessor).label(""),multi=fc.series.multi().key(function(series,index){return series.isLine?index:series}).series([gridlines,currentSeries.option,closeLine,crosshair]).mapping(function(series){switch(series){case closeLine:return[this.data[this.data.length-1]];case crosshair:return crosshairData;default:return this.data}}),xScale=fc.scale.dateTime(),yScale=d3.scale.linear(),primaryChart=fc.chart.cartesian(xScale,yScale).xTicks(0).yOrient("right").margin({top:0,left:0,bottom:0,right:yAxisWidth}),movingAverage=fc.indicator.algorithm.movingAverage(),bollingerAlgorithm=fc.indicator.algorithm.bollingerBands(),selectorsChanged=function(model){currentSeries=model.series,currentYValueAccessor=model.yValueAccessor.option,currentIndicators=model.indicators,updateYValueAccessorUsed(),updateMultiSeries(multi.series),primaryChart.yTickFormat(model.product.priceFormat),model.selectorsChanged=!1};
// Call when the main layout is modified
return d3.rebind(primary,dispatch,"on"),primary.dimensionChanged=function(container){zoomWidth=parseInt(container.style("width"))-yAxisWidth},primary}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.chart.rsi=function(){function rsi(selection){var model=selection.datum();algorithm(model.data),chart.trackingLatest(model.trackingLatest).padding(model.padding).xDomain(model.viewDomain).yDomain([0,100]),selection.datum(model.data).call(chart)}var dispatch=d3.dispatch(sc.event.viewChange),renderer=fc.indicator.renderer.relativeStrengthIndex(),algorithm=fc.indicator.algorithm.relativeStrengthIndex(),tickValues=[renderer.lowerValue(),50,renderer.upperValue()],chart=sc.chart.secondary().series([renderer]).yTickValues(tickValues).on(sc.event.viewChange,function(domain){dispatch[sc.event.viewChange](domain)});return d3.rebind(rsi,dispatch,"on"),rsi.dimensionChanged=function(container){chart.dimensionChanged(container)},rsi}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.chart.secondary=function(){function secondary(selection){selection.each(function(data){var container=d3.select(this).call(chart),zoom=sc.behavior.zoom(zoomWidth).scale(xScale).padding(padding).trackingLatest(trackingLatest).on("zoom",function(domain){dispatch[sc.event.viewChange](domain)});container.select(".plot-area-container").datum({data:selection.datum()}).call(zoom)})}var zoomWidth,dispatch=d3.dispatch(sc.event.viewChange),xScale=fc.scale.dateTime(),yScale=d3.scale.linear(),padding=0,trackingLatest=!0,yAxisWidth=60,multi=fc.series.multi(),chart=fc.chart.cartesian(xScale,yScale).plotArea(multi).xTicks(0).yOrient("right").margin({top:0,left:0,bottom:0,right:yAxisWidth});return secondary.padding=function(x){return arguments.length?(padding=x,secondary):padding},secondary.trackingLatest=function(x){return arguments.length?(trackingLatest=x,secondary):trackingLatest},d3.rebind(secondary,dispatch,"on"),d3.rebind(secondary,multi,"series","mapping","decorate"),d3.rebind(secondary,chart,"yTickValues","yTickFormat","yTicks","xDomain","yDomain"),secondary.dimensionChanged=function(container){zoomWidth=parseInt(container.style("width"))-yAxisWidth},secondary}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.chart.volume=function(){function volume(selection){selection.each(function(model){var maxYExtent=d3.max(model.data,function(d){return d3.max([d.volume])}),minYExtent=d3.min(model.data,function(d){return d3.min([d.volume])}),paddedYExtent=sc.util.domain.padYDomain([minYExtent,maxYExtent],.04);chart.yTickFormat(model.product.volumeFormat).trackingLatest(model.trackingLatest).padding(model.padding).xDomain(model.viewDomain).yDomain(paddedYExtent),selection.datum(model.data).call(chart)})}var dispatch=d3.dispatch(sc.event.viewChange),volumeBar=fc.series.bar().yValue(function(d){return d.volume}),chart=sc.chart.secondary().series([volumeBar]).yTicks(4).on(sc.event.viewChange,function(domain){dispatch[sc.event.viewChange](domain)});return d3.rebind(volume,dispatch,"on"),volume.dimensionChanged=function(container){chart.dimensionChanged(container)},volume}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.chart.xAxis=function(){function preventTicksMoreFrequentThanPeriod(period){var scaleTickSeconds=(xScale.ticks()[1]-xScale.ticks()[0])/1e3;scaleTickSeconds<period.seconds?xAxis.ticks(period.d3TimeInterval.unit,period.d3TimeInterval.value):xAxis.ticks(6)}function xAxisChart(selection){var model=selection.datum();xScale.domain(model.viewDomain),preventTicksMoreFrequentThanPeriod(model.period),selection.call(xAxis)}var xScale=fc.scale.dateTime(),xAxis=d3.svg.axis().scale(xScale).orient("bottom");return xAxisChart.dimensionChanged=function(container){xScale.range([0,parseInt(container.style("width"))])},xAxisChart}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.menu.generator.buttonGroup=function(){function buttonGroup(selection){selection.each(function(model){var selection=d3.select(this);dataJoin(selection,model.options).classed("active",function(option){return option.isSelected}).classed("btn btn-primary",!0).text(function(option){return option.displayString}).on("click",dispatch.optionChange)})}var dispatch=d3.dispatch("optionChange"),dataJoin=fc.util.dataJoin().selector(".button").element("label").attr("class","button");return d3.rebind(buttonGroup,dispatch,"on"),buttonGroup}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.menu.generator.dropdownGroup=function(){function dropdownGroup(selection){var selectedIndex=selection.datum().selectedIndex||0,model=selection.datum(),ul=dataJoin(selection,[model.options]);ul.attr("class","dropdown-menu");var li=ul.selectAll("li").data(fc.util.fn.identity);li.enter().append("li").on("click",dispatch.optionChange).append("a").attr("href","#"),li.select("a").text(function(d){return d.displayString}),selection.select("#product-dropdown-button").html(function(d){return model.options[selectedIndex].displayString+'<span class="caret"></span>'})}var dispatch=d3.dispatch("optionChange"),dataJoin=fc.util.dataJoin().selector("ul").element("ul");return d3.rebind(dropdownGroup,dispatch,"on"),dropdownGroup}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.menu.generator.tabGroup=function(){function tabGroup(selection){var selectedIndex=selection.datum().selectedIndex||0,ul=dataJoin(selection,[selection.datum().options]);ul.enter().append("ul");var li=ul.selectAll("li").data(fc.util.fn.identity);li.enter().append("li").append("a").attr("href","#").on("click",dispatch.tabClick),li.classed("active",function(d,i){return i===selectedIndex}).select("a").text(function(option){return option.displayString})}var dispatch=d3.dispatch("tabClick"),dataJoin=fc.util.dataJoin().selector("ul").element("ul");return d3.rebind(tabGroup,dispatch,"on"),tabGroup}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.menu.head=function(){var dispatch=d3.dispatch(sc.event.toggleSlideout,sc.event.dataProductChange,sc.event.dataPeriodChange),dataProductDropdown=sc.menu.generator.dropdownGroup().on("optionChange",function(product){dispatch[sc.event.dataProductChange](product)}),dataPeriodSelector=sc.menu.generator.tabGroup().on("tabClick",function(period){dispatch[sc.event.dataPeriodChange](period)}),head=function(selection){selection.each(function(model){var selection=d3.select(this),products=model.products;selection.select("#product-dropdown").datum({options:products.map(sc.menu.productAdaptor),selectedIndex:products.indexOf(model.selectedProduct)}).call(dataProductDropdown);var periods=model.selectedProduct.periods;selection.select("#period-selector").classed("hidden",periods.length<=1).datum({options:periods.map(sc.menu.periodAdaptor),selectedIndex:periods.indexOf(model.selectedPeriod)}).call(dataPeriodSelector),selection.select("#toggle-button").on("click",function(){dispatch[sc.event.toggleSlideout]()})})};return d3.rebind(head,dispatch,"on")}}(d3,fc,sc),function(d3,fc,sc){"use strict";
// TODO: Move to /model
sc.menu.option=function(displayString,valueString,option){return{displayString:displayString,// TODO: is 'displayName' better?
...
isSelected:!1}},/**
     ...
     */
sc.menu.productAdaptor=function(product){return{displayString:product.display,option:product}},/**
     ...
     */
sc.menu.periodAdaptor=function(period){return{displayString:period.display,option:period}}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.menu.side=function(){var dispatch=d3.dispatch(sc.event.primaryChartSeriesChange,sc.event.primaryChartYValueAccessorChange,sc.event.primaryChartIndicatorChange,sc.event.secondaryChartChange),primaryChartSeriesButtons=sc.menu.generator.buttonGroup().on("optionChange",dispatch[sc.event.primaryChartSeriesChange]),primaryChartYValueAccessorButtons=sc.menu.generator.buttonGroup().on("optionChange",dispatch[sc.event.primaryChartYValueAccessorChange]),primaryChartIndicatorToggle=sc.menu.generator.buttonGroup().on("optionChange",dispatch[sc.event.primaryChartIndicatorChange]),secondaryChartToggle=sc.menu.generator.buttonGroup().on("optionChange",dispatch[sc.event.secondaryChartChange]),side=function(selection){selection.each(function(model){var selection=d3.select(this);selection.select(".series-buttons").datum({options:model.seriesOptions}).call(primaryChartSeriesButtons),selection.select(".y-value-accessor-buttons").datum({options:model.yValueAccessorOptions}).call(primaryChartYValueAccessorButtons),selection.select(".indicator-buttons").datum({options:model.indicatorOptions}).call(primaryChartIndicatorToggle),selection.select(".secondary-chart-buttons").datum({options:model.secondaryChartOptions}).call(secondaryChartToggle)})};return d3.rebind(side,dispatch,"on")}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.util.domain.centerOnDate=function(domain,data,centerDate){var dataExtent=fc.util.extent().fields("date")(data),domainTimes=domain.map(function(d){return d.getTime()}),domainTimeDifference=(d3.max(domainTimes)-d3.min(domainTimes))/1e3;if(centerDate.getTime()<dataExtent[0]||centerDate.getTime()>dataExtent[1])return[new Date(d3.min(domainTimes)),new Date(d3.max(domainTimes))];var centeredDataDomain=[d3.time.second.offset(centerDate,-domainTimeDifference/2),d3.time.second.offset(centerDate,domainTimeDifference/2)],timeShift=0;return centeredDataDomain[1].getTime()>dataExtent[1].getTime()?timeShift=(dataExtent[1].getTime()-centeredDataDomain[1].getTime())/1e3:centeredDataDomain[0].getTime()<dataExtent[0].getTime()&&(timeShift=(dataExtent[0].getTime()-centeredDataDomain[0].getTime())/1e3),[d3.time.second.offset(centeredDataDomain[0],timeShift),d3.time.second.offset(centeredDataDomain[1],timeShift)]}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.util.domain.filterDataInDateRange=function(domain,data){var startDate=d3.min(domain,function(d){return d.getTime()}),endDate=d3.max(domain,function(d){return d.getTime()}),dataSortedByDate=data.sort(function(a,b){return a.date-b.date}),bisector=d3.bisector(function(d){return d.date}),filteredData=data.slice(
// Pad and clamp the bisector values to ensure extents can be calculated
Math.max(0,bisector.left(dataSortedByDate,startDate)-1),Math.min(bisector.right(dataSortedByDate,endDate)+1,dataSortedByDate.length));return filteredData}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.util.domain.moveToLatest=function(domain,data,ratio){arguments.length<3&&(ratio=1);var dataExtent=fc.util.extent().fields("date")(data),dataTimeExtent=(dataExtent[1].getTime()-dataExtent[0].getTime())/1e3,domainTimes=domain.map(function(d){return d.getTime()}),scaledDomainTimeDifference=ratio*(d3.max(domainTimes)-d3.min(domainTimes))/1e3,scaledLiveDataDomain=dataTimeExtent>scaledDomainTimeDifference?[d3.time.second.offset(dataExtent[1],-scaledDomainTimeDifference),dataExtent[1]]:dataExtent;return scaledLiveDataDomain}}(d3,fc,sc),function(d3,fc,sc){"use strict";function domainBeforeStartOfData(domain,data){var earliestViewedTime=domain[0].getTime(),earliestDatumTime=data[0].date.getTime();return earliestDatumTime>=earliestViewedTime}function domainAfterEndOfData(domain,data){var latestViewedTime=domain[1].getTime(),lastDatumTime=data[data.length-1].date.getTime();return latestViewedTime>=lastDatumTime}sc.util.domain.padTimeExtent=function(domain,data,padding){
// Adds a padding of period to ends of domain
var paddedDomain=domain;return domainBeforeStartOfData(domain,data)&&(paddedDomain[0]=d3.time.second.offset(new Date(domain[0]),-padding)),domainAfterEndOfData(domain,data)&&(paddedDomain[1]=d3.time.second.offset(new Date(domain[1]),+padding)),paddedDomain}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.util.domain.padYDomain=function(yExtent,paddingPercentage){var paddingArray=Array.isArray(paddingPercentage)?paddingPercentage:[paddingPercentage,paddingPercentage],orderedYExtentDifference=yExtent[1]-yExtent[0];return[yExtent[0]-orderedYExtentDifference*paddingArray[0],yExtent[1]+orderedYExtentDifference*paddingArray[1]]}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.util.domain.trackingLatestData=function(domain,data){var latestViewedTime=d3.max(domain,function(d){return d.getTime()}),latestDatumTime=d3.max(data,function(d){return d.date.getTime()});return latestViewedTime===latestDatumTime}}(d3,fc,sc),function(d3,fc,sc){"use strict";var renderedOnce=!1;sc.util.layout=function(containers,charts){function getSecondaryContainer(i){return containers.secondaries.filter(function(d,i){return i===iChart})}for(var secondaryChartsShown=0,j=0;j<charts.secondaries.length;j++)charts.secondaries[j]&&secondaryChartsShown++;containers.secondaries.filter(function(d,i){return secondaryChartsShown>i}).style("flex","1"),containers.secondaries.filter(function(d,i){return i>=secondaryChartsShown}).style("flex","0");var headRowHeight=parseInt(containers.app.select(".head-row").style("height"),10);renderedOnce||(headRowHeight+=parseInt(containers.app.select(".head-row").style("padding-top"),10)+parseInt(containers.app.select(".head-row").style("padding-bottom"),10)+parseInt(containers.app.select(".head-row").style("margin-bottom"),10),renderedOnce=!0);var useableScreenHeight=window.innerHeight-headRowHeight;containers.charts.style("height",useableScreenHeight+"px"),charts.xAxis.dimensionChanged(containers.xAxis),charts.navbar.dimensionChanged(containers.navbar),charts.primary.dimensionChanged(containers.primary);for(var iChart=0;iChart<charts.secondaries.length;iChart++)charts.secondaries[iChart].option.dimensionChanged(getSecondaryContainer(iChart))}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.data.callbackInvalidator=function(){function callbackInvalidator(callback){var id=++n;return function(err,data){n>id||callback(err,data)}}var n=0;return callbackInvalidator.invalidateCallback=function(){return n++,callbackInvalidator},callbackInvalidator}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.data.dataInterface=function(){function updateHistoricFeedDateRangeToPresent(period){var currDate=new Date,startDate=d3.time.second.offset(currDate,-candlesOfData*period);historicFeed.start(startDate).end(currDate)}function newBasketReceived(basket,data){data[data.length-1].date.getTime()!==basket.date.getTime()?data.push(basket):data[data.length-1]=basket}function liveCallback(data){return function(socketEvent,latestBasket){"message"===socketEvent.type&&latestBasket&&newBasketReceived(latestBasket,data),dispatch[sc.event.messageReceived](socketEvent,data)}}function dataInterface(period){dataInterface.invalidate(),historicFeed.granularity(period),ohlcConverter.period(period),updateHistoricFeedDateRangeToPresent(period);var currentData=[];historicFeed(callbackGenerator(function(err,data){err||(currentData=data.reverse(),ohlcConverter(liveCallback(currentData),currentData[currentData.length-1])),dispatch[sc.event.dataLoaded](err,currentData)}))}var historicFeed=fc.data.feed.coinbase(),callbackGenerator=sc.data.callbackInvalidator(),ohlcConverter=sc.data.feed.coinbase.ohlcWebSocketAdaptor(),dataGenerator=fc.data.random.financial(),dispatch=d3.dispatch(sc.event.messageReceived,sc.event.dataLoaded),candlesOfData=200;return dataInterface.generateDailyData=function(){dataInterface.invalidate();var now=new Date;now.setHours(0,0,0,0);var millisecondsPerDay=864e5;dataGenerator.startDate(new Date(now-(candlesOfData-1)*millisecondsPerDay));var dataGenerated=dataGenerator(candlesOfData);dispatch[sc.event.dataLoaded](null,dataGenerated)},dataInterface.invalidate=function(){return ohlcConverter.close(),callbackGenerator.invalidateCallback(),dataInterface},d3.rebind(dataInterface,dispatch,"on"),dataInterface}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.data.feed.coinbase.ohlcWebSocketAdaptor=function(){function createNewBasket(datum,time){return{date:time,open:datum.price,close:datum.price,low:datum.price,high:datum.price,volume:datum.volume}}function updateBasket(basket,datum){null==basket&&(basket=createNewBasket(datum,datum.date));var latestTime=datum.date.getTime(),startTime=basket.date.getTime(),msPeriod=1e3*period;if(latestTime>startTime+msPeriod){var timeIntoCurrentPeriod=(latestTime-startTime)%msPeriod,newTime=latestTime-timeIntoCurrentPeriod;basket=createNewBasket(datum,new Date(newTime))}else
// Update current basket
basket.high=Math.max(basket.high,datum.price),basket.low=Math.min(basket.low,datum.price),basket.volume+=datum.volume,basket.close=datum.price;return basket}function ohlcWebSocketAdaptor(cb,initialBasket){var basket=initialBasket;liveFeed(function(err,datum){datum&&(basket=updateBasket(basket,datum)),cb(err,basket)})}
...
// In seconds
var period=86400,liveFeed=sc.data.feed.coinbase.webSocket();return ohlcWebSocketAdaptor.period=function(x){return arguments.length?(period=x,ohlcWebSocketAdaptor):period},d3.rebind(ohlcWebSocketAdaptor,liveFeed,"product","messageType","close"),ohlcWebSocketAdaptor}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.data.feed.coinbase.webSocket=function(){function webSocket(cb){webSocket.close(),coinbaseSocket=new WebSocket("wss://ws-feed.exchange.coinbase.com");var msg={type:"subscribe",product_id:product};coinbaseSocket.onopen=function(event){coinbaseSocket.send(JSON.stringify(msg)),cb(event,null)},coinbaseSocket.onmessage=function(event){var messageData=JSON.parse(event.data);if(messageData.type===msgType){var datum={};datum.date=new Date(messageData.time),datum.price=parseFloat(messageData.price),datum.volume=parseFloat(messageData.size),cb(event,datum)}},coinbaseSocket.onerror=function(event){cb(event,null)},coinbaseSocket.onclose=function(event){cb(event,null)}}var product="BTC-USD",msgType="match",coinbaseSocket=null;return webSocket.close=function(){return coinbaseSocket&&coinbaseSocket.close(),webSocket},webSocket.messageType=function(x){return arguments.length?(msgType=x,webSocket):msgType},webSocket.product=function(x){return arguments.length?(product=x,webSocket):product},webSocket}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.behavior.zoom=function(width){function controlPan(zoomExtent){
// Don't pan off sides
// Don't pan off sides
return zoomExtent[0]>=0?-zoomExtent[0]:zoomExtent[1]<=0?-zoomExtent[1]:0}function controlZoom(zoomExtent){
// If zooming, and about to pan off screen, do nothing
return zoomExtent[0]>0&&zoomExtent[1]<0}function translateXZoom(translation){var tx=zoomBehavior.translate()[0];tx+=translation,zoomBehavior.translate([tx,0])}function zoom(selection){zoomBehavior.x(scale).on("zoom",function(){var xExtent=fc.util.extent().fields("date")(selection.datum().data),paddedXExtent=padding?sc.util.domain.padTimeExtent(xExtent,selection.datum().data,padding):xExtent,width=selection.attr("layout-width"),min=scale(paddedXExtent[0]),max=scale(paddedXExtent[1]),maxDomainViewed=controlZoom([min,max-width]),panningRestriction=controlPan([min,max-width]);translateXZoom(panningRestriction);var panned=1===zoomBehavior.scale(),zoomed=1!==zoomBehavior.scale();if(panned&&allowPan||zoomed&&allowZoom){var unpaddedDomain=padding?sc.util.domain.padTimeExtent(scale.domain(),selection.datum().data,-padding):scale.domain();maxDomainViewed?unpaddedDomain=fc.util.extent(selection.datum().data,["date"]):zoomed&&trackingLatest&&(unpaddedDomain=sc.util.domain.moveToLatest(unpaddedDomain,selection.datum().data)),dispatch.zoom(unpaddedDomain)}else
// Resets zoomBehaviour
zoomBehavior.translate([0,0]),zoomBehavior.scale(1)}),selection.call(zoomBehavior)}var scale,dispatch=d3.dispatch("zoom"),zoomBehavior=d3.behavior.zoom(),allowPan=!0,allowZoom=!0,padding=0,trackingLatest=!0;return zoom.allowPan=function(x){return arguments.length?(allowPan=x,zoom):allowPan},zoom.allowZoom=function(x){return arguments.length?(allowZoom=x,zoom):allowZoom},zoom.padding=function(x){return arguments.length?(padding=x,zoom):padding},zoom.trackingLatest=function(x){return arguments.length?(trackingLatest=x,zoom):trackingLatest},zoom.scale=function(x){return arguments.length?(scale=x,zoom):scale},d3.rebind(zoom,dispatch,"on"),zoom}}(d3,fc,sc),function(d3,fc,sc){"use strict";/* Credit to Chris Price for this optimisation
    http://blog.scottlogic.com/2015/08/06/an-adventure-in-svg-filter-land.html
    */
sc.series.candlestick=function(){var xScale=fc.scale.dateTime(),yScale=d3.scale.linear(),barWidth=fc.util.fractionalBarWidth(.75),xValue=function(d,i){return d.date},xValueScaled=function(d,i){return xScale(xValue(d,i))},yLowValue=function(d){return d.low},yHighValue=function(d){return d.high},yCloseValue=function(d,i){return d.close},candlestickSvg=fc.svg.candlestick().x(function(d){return xScale(d.date)}).open(function(d){return yScale(d.open)}).high(function(d){return yScale(yHighValue(d))}).low(function(d){return yScale(yLowValue(d))}).close(function(d){return yScale(d.close)}),upDataJoin=fc.util.dataJoin().selector("path.up").element("path").attr("class","up"),downDataJoin=fc.util.dataJoin().selector("path.down").element("path").attr("class","down"),candlestick=function(selection){selection.each(function(data){candlestickSvg.width(barWidth(data.map(xValueScaled)));var upData=data.filter(function(d){return d.open<d.close}),downData=data.filter(function(d){return d.open>=d.close});upDataJoin(this,[upData]).attr("d",candlestickSvg),downDataJoin(this,[downData]).attr("d",candlestickSvg)})};return candlestick.xScale=function(x){return arguments.length?(xScale=x,candlestick):xScale},candlestick.yScale=function(x){return arguments.length?(yScale=x,candlestick):yScale},candlestick.xValue=function(x){return arguments.length?(xValue=x,candlestick):xValue},candlestick.yLowValue=function(x){return arguments.length?(yLowValue=x,candlestick):yLowValue},candlestick.yHighValue=function(x){return arguments.length?(yHighValue=x,candlestick):yHighValue},candlestick.yCloseValue=function(x){return arguments.length?(yCloseValue=x,candlestick):yCloseValue},candlestick.width=function(data){return barWidth(data.map(xValueScaled))},candlestick}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.app=function(){function renderInternal(){layoutRedrawnInNextRender&&containers.suspendLayout(!1),containers.primary.datum(primaryChartModel).call(charts.primary),containers.legend.datum(legendModel).call(charts.legend),containers.secondaries.datum(secondaryChartModel).filter(function(d,i){return i<charts.secondaries.length}).each(function(d,i){d3.select(this).attr("class","secondary-container "+charts.secondaries[i].valueString).call(charts.secondaries[i].option)}),containers.xAxis.datum(xAxisModel).call(charts.xAxis),containers.navbar.datum(navModel).call(charts.navbar),containers.app.select(".head-menu").datum(headMenuModel).call(headMenu),containers.app.select(".sidebar-menu").datum(sideMenuModel).call(sideMenu),layoutRedrawnInNextRender&&(containers.suspendLayout(!0),layoutRedrawnInNextRender=!1)}function updateLayout(){layoutRedrawnInNextRender=!0,sc.util.layout(containers,charts)}function initialiseResize(){d3.select(window).on("resize",function(){updateLayout(),render()})}function onViewChange(unpaddedDomain){var trackingLatest=sc.util.domain.trackingLatestData(unpaddedDomain,primaryChartModel.data);primaryChartModel.trackingLatest=trackingLatest,secondaryChartModel.trackingLatest=trackingLatest,navModel.trackingLatest=trackingLatest;var viewDomain=primaryChartModel.series.option.isPadded?sc.util.domain.padTimeExtent(unpaddedDomain,primaryChartModel.data,xAxisModel.period.seconds/2):unpaddedDomain;primaryChartModel.viewDomain=viewDomain,secondaryChartModel.viewDomain=viewDomain,xAxisModel.viewDomain=viewDomain,navModel.viewDomain=viewDomain,render()}function onCrosshairChange(dataPoint){legendModel.data=dataPoint,render()}function resetToLatest(){var data=primaryChartModel.data,dataDomain=fc.util.extent().fields("date")(data),navTimeDomain=sc.util.domain.moveToLatest(dataDomain,data,.2);onViewChange(navTimeDomain)}function updateModelData(data){primaryChartModel.data=data,secondaryChartModel.data=data,navModel.data=data}function updateModelSelectedProduct(product){headMenuModel.selectedProduct=product,primaryChartModel.product=product,secondaryChartModel.product=product,legendModel.product=product}function updateModelSelectedPeriod(period){headMenuModel.selectedPeriod=period,xAxisModel.period=period,legendModel.period=period}function initialisePrimaryChart(){return sc.chart.primary().on(sc.event.crosshairChange,onCrosshairChange).on(sc.event.viewChange,onViewChange)}function initialiseNav(){return sc.chart.nav().on(sc.event.viewChange,onViewChange).on(sc.event.resetToLatest,resetToLatest)}function initialiseDataInterface(){return sc.data.dataInterface().on(sc.event.messageReceived,function(socketEvent,data){if("error"===socketEvent.type||"close"===socketEvent.type&&1e3!==socketEvent.code)console.log("Error loading data from coinbase websocket: "+socketEvent.type+" "+socketEvent.code);else if("message"===socketEvent.type&&(updateModelData(data),primaryChartModel.trackingLatest)){var newDomain=sc.util.domain.moveToLatest(primaryChartModel.viewDomain,primaryChartModel.data);onViewChange(newDomain)}}).on(sc.event.dataLoaded,function(err,data){err?console.log("Error getting historic data: "+err):(updateModelData(data),legendModel.data=null,resetToLatest())})}function initialiseHeadMenu(dataInterface){return sc.menu.head().on(sc.event.dataProductChange,function(product){updateModelSelectedProduct(product.option),updateModelSelectedPeriod(product.option.periods[0]),product.option===bitcoin?dataInterface(product.option.periods[0].seconds):product.option===generated&&dataInterface.generateDailyData(),render()}).on(sc.event.dataPeriodChange,function(period){updateModelSelectedPeriod(period.option),dataInterface(period.option.seconds),render()}).on(sc.event.toggleSlideout,function(){containers.app.selectAll(".row-offcanvas-right").classed("active",!containers.app.selectAll(".row-offcanvas-right").classed("active"))})}function selectOption(option,options){options.forEach(function(option){option.isSelected=!1}),option.isSelected=!0}function initialiseSideMenu(){return sc.menu.side().on(sc.event.primaryChartSeriesChange,function(series){var unpaddedDomain=primaryChartModel.series.option.isPadded?sc.util.domain.padTimeExtent(primaryChartModel.viewDomain,primaryChartModel.data,-xAxisModel.period.seconds/2):primaryChartModel.viewDomain;primaryChartModel.series=series,primaryChartModel.padding=series.option.isPadded?xAxisModel.period.seconds/2:0,secondaryChartModel.padding=series.option.isPadded?xAxisModel.period.seconds/2:0,xAxisModel.padding=series.option.isPadded?xAxisModel.period.seconds/2:0,navModel.padding=series.option.isPadded?xAxisModel.period.seconds/2:0,selectOption(series,sideMenuModel.seriesOptions),onViewChange(unpaddedDomain)}).on(sc.event.primaryChartYValueAccessorChange,function(yValueAccessor){primaryChartModel.yValueAccessor=yValueAccessor,selectOption(yValueAccessor,sideMenuModel.yValueAccessorOptions),render()}).on(sc.event.primaryChartIndicatorChange,function(indicator){indicator.isSelected=!indicator.isSelected,primaryChartModel.indicators=sideMenuModel.indicatorOptions.filter(function(option){return option.isSelected}),render()}).on(sc.event.secondaryChartChange,function(chart){chart.isSelected=!chart.isSelected,charts.secondaries=sideMenuModel.secondaryChartOptions.filter(function(option){return option.isSelected}),
// TODO: This doesn't seem to be a concern of menu.
charts.secondaries.forEach(function(chartOption){chartOption.option.on(sc.event.viewChange,onViewChange)}),
// TODO: Remove .remove! (could a secondary chart group component manage this?).
containers.secondaries.selectAll("*").remove(),updateLayout(),render()})}var headMenu,sideMenu,app={},appContainer=d3.select("#app-container"),chartsContainer=appContainer.select("#charts-container"),containers={app:appContainer,charts:chartsContainer,primary:chartsContainer.select("#primary-container"),secondaries:chartsContainer.selectAll(".secondary-container"),xAxis:chartsContainer.select("#x-axis-container"),navbar:chartsContainer.select("#navbar-row"),legend:appContainer.select("#legend"),suspendLayout:function(value){var self=this;Object.keys(self).forEach(function(key){"function"!=typeof self[key]&&self[key].layoutSuspended(value)})}},day1=sc.model.period({display:"Daily",seconds:86400,d3TimeInterval:{unit:d3.time.day,value:1},timeFormat:"%b %d"}),hour1=sc.model.period({display:"1 Hr",seconds:3600,d3TimeInterval:{unit:d3.time.hour,value:1},timeFormat:"%b %d %Hh"}),minute5=sc.model.period({display:"5 Min",seconds:300,d3TimeInterval:{unit:d3.time.minute,value:5},timeFormat:"%H:%M"}),minute1=sc.model.period({display:"1 Min",seconds:60,d3TimeInterval:{unit:d3.time.minute,value:1},timeFormat:"%H:%M"}),generated=sc.model.product({display:"Data Generator",volumeFormat:".3s",periods:[day1]}),bitcoin=sc.model.product({display:"Bitcoin",volumeFormat:".2f",periods:[minute1,minute5,hour1]}),primaryChartModel=sc.model.primaryChart(generated),secondaryChartModel=sc.model.secondaryChart(generated),sideMenuModel=sc.model.menu.side(),xAxisModel=sc.model.xAxis(day1),navModel=sc.model.nav(),headMenuModel=sc.model.headMenu([generated,bitcoin],generated,day1),legendModel=sc.model.legend(generated,day1),charts={primary:void 0,secondaries:[],xAxis:sc.chart.xAxis(),navbar:void 0,legend:sc.chart.legend()},render=fc.util.render(renderInternal),layoutRedrawnInNextRender=!0;return app.run=function(){charts.primary=initialisePrimaryChart(),charts.navbar=initialiseNav();var dataInterface=initialiseDataInterface();headMenu=initialiseHeadMenu(dataInterface),sideMenu=initialiseSideMenu(),updateLayout(),initialiseResize(),dataInterface.generateDailyData()},app}}(d3,fc,sc),function(d3,fc,sc){"use strict";sc.app().run()}(d3,fc,sc);