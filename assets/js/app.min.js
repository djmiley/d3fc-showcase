...
return zoomExtent[0]>0&&zoomExtent[1]<0}function translateXZoom(translation){var tx=zoomBehavior.translate()[0];tx+=translation,zoomBehavior.translate([tx,0])}function zoom(selection){zoomBehavior.x(scale).on("zoom",function(){var xExtent=fc.util.extent().fields("date")(selection.datum().data),paddedXExtent=padding?sc.util.domain.padTimeExtent(xExtent,padding):xExtent,min=scale(paddedXExtent[0]),max=scale(paddedXExtent[1]),maxDomainViewed=controlZoom([min,max-width]),panningRestriction=controlPan([min,max-width]);translateXZoom(panningRestriction);var panned=1===zoomBehavior.scale(),zoomed=1!==zoomBehavior.scale();if(panned&&allowPan||zoomed&&allowZoom){var unpaddedDomain=padding?sc.util.domain.padTimeExtent(scale.domain(),-padding):scale.domain();maxDomainViewed?unpaddedDomain=fc.util.extent().fields("date")(selection.datum().data):zoomed&&trackingLatest&&(unpaddedDomain=sc.util.domain.moveToLatest(unpaddedDomain,selection.datum().data)),dispatch.zoom(unpaddedDomain)}else
